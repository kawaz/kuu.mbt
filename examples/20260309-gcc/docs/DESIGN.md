...
| 強制ON（冪等） | `True("force")` | `--force-diagnostics-color` |
| デフォルトにリセット | `Reset("reset")` | `--reset-debug` |
...
### 位置引数・特殊
- ソースファイル (rest + stop_before)
- ※ dashdash は rest() との競合により未使用（DR-001 参照）

## 制約

| 制約種別 | 対象 |
|---|---|
| exclusive | `--compile` / `--assemble-only` / `--preprocess` |
