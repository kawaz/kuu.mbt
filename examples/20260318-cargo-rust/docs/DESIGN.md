...
### 3. test / t
- **kuu 機能**: positional (TESTNAME), dashdash, variation_false (--no-run, --no-fail-fast)
...
### 5. add
- **kuu 機能**: rest (複数 positional), exclusive (source group, section group), variation_false
...
### 7. clean
- **kuu 機能**: シンプルなサブコマンド、variation_false
...
### 9. doc / d
- **kuu 機能**: variation_false (--no-deps), flag
...
| variation_false | --no-run, --no-fail-fast, --no-deps, --no-default-features, --no-optional |
