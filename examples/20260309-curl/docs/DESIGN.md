...
`max-redirs` は `default=50` で定義（curl 公式のデフォルト値）。
print_int ヘルパーは非ゼロ値を表示するため、未指定時でも `50` と表示される。
ユーザーが明示的に指定したかは `is_set()` で区別可能だが、デモの簡潔さを優先してゼロ判定を採用。

### exclusive 制約の不採用

curl では `--silent` と `--verbose` は共存可能であり、排他制約は不正確。
また kuu の `exclusive` は `is_set` ベースで判定するため、`variation_false` 付きフラグの否定形（`--no-verbose` 等）も `is_set=true` となり、`--silent --no-verbose` のような実質非衝突ケースでも排他エラーが発火する問題がある。
そのため本デモでは exclusive を使用しない。exclusive のデモは他の example（mygit, mydocker）で十分カバーされている。
