...
- **file コンビネータ** — `string_opt` / `int_opt` と並ぶファイルパス特化コンビネータ。`implicit_value` との組み合わせで3値パターン（未指定=None / フラグのみ=デフォルトパス / 値指定=指定パス）を自然に表現。`default=file(arg="~/.config/xxx/xxx.toml")` のように、デフォルト値にもファイル存在チェック等のバリデーションを適用できる構想。clap で `num_args=0..=1` + サブコマンド衝突が起きる問題を、kuu は ExactNode 走査 + コンビネータ設計で構造的に回避
