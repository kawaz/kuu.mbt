...
- **ユーザーは `opt.get()` で型付きの値を取得** — Opt[T] がレンズ/キーとして機能。戻り値は `T?`（パース前なら `None`）。`result.get(opt)` / `p.get(opt)` は非推奨
...
pub(all) struct AliasSource {
  ...
  make_eq_fallback : ((String) -> ExactNode)?  // eq_split フォールバック用ファクトリ（implicit_value 付きのみ）
}
```

alias コンビネータが alias_sources に登録し、チェーン alias（alias の alias）時の target_was_set 伝搬と ExactNode ファクトリの保存に使用。
...
各コンビネータ（flag, string 等）は ExactNode を生成して `parser.nodes` に直接 push する。
...
#### 直交プリミティブと合成パターン (DR-037)

3 つの直交プリミティブで alias / variation / proxy が全て合成可能:

| プリミティブ | 関心事 | 役割 | 状態 |
|-------------|--------|------|------|
| `clone(opt, name)` | アイデンティティ | 構造コピー（新 Ref、フィルタ参照共有） | 未実装 |
| `link(opt, val_source)` | 値 | Ref 共有（書き込み先を統合） | 未実装 |
| `adjust(opt, ...)` | 振る舞い | フィルタチェーンの前後に挿入 | 未実装 |
...
合成コンビネータ:
- **alias** — `p.alias(name, target)` で値共有 + is_set 独立の別名を登録。チェーン alias 対応。global alias は子パーサに伝搬【実装済み】
- **deprecated** — `p.deprecated(name, target, msg)` で非推奨別名を登録。alias + post_hook 方式。`deprecated_warnings()` で警告取得。サブコマンド→親への再帰的伝搬対応【実装済み、DR-040】
- **adjust** — フィルタ挿入。alias なしでも単独使用可。LongVariation（Toggle/True/False/Reset/Unset）は adjust の引数プリセット定数。deprecated は post_hook で代替済み【未実装、Opt[T] setter 追加後に実装予定】
...
| レイヤー | 責務 | 実装ファイル |
|---------|------|------------|
| Tokenizer | short_combine, eq_split — 横断的な引数前処理 | parse.mbt |
| Core | ExactNode（完全一致名マッチ + try_reduce）+ 消費ループ + プリミティブ（exact, clone, link, adjust） | parse.mbt, nodes.mbt |
| Pattern | make_or_node, alias, variation, adjust — 合成パターン | nodes.mbt |
| Convention | expand_and_register — --name + aliases + shorts + variations 展開 | parser.mbt |
| Sugar | flag(), string(), int(), count(), append_string(), append_int(), custom(), positional(), rest(), cmd() | options.mbt, commands.mbt, positionals.mbt, dashdash.mbt |

Design rationale (DR-012, DR-037): 旧設計のツリー構造は廃止。各レイヤーが ExactNode の生成と Parser への登録という
単純な操作に統一された。make_or_node は最長一致ロジックを内包し、choices + implicit_value のような
複合パターンを単一の ExactNode に見せる。DR-037 で 3 つの直交プリミティブ（clone/link/adjust）を導入し、
alias = clone + link、variation = alias + adjust として統合。deprecated は adjust の記録パターン。
...
Design rationale (DR-037): Variation は「alias + フィルタ合成」として再定義可能。
clone して pre/post/accum を variation 固有の振る舞いで wrap する合成パターン。
現在のノード直接生成方式は動作するが、将来的に alias/proxy プリミティブに統合可能。
...
| Single(String) | `string()` | custom 経由: make_reducer(identity, replace, initial) |
| Single(Int) | `int()` | custom 経由: make_reducer(parse_int, replace, initial) |
| Append | `append_string()` | make_value_node で直接構築 |
| choices | `string(choices=[...])` | make_reducer(one_of, replace, initial) |
| implicit_value | `string(implicit_value=...)` | make_or_node で composite 構築 |

Design rationale: Flag/Count は単純すぎるため ReduceCtx を経由せず ExactNode を直接構築する。
値オプション（string, int の通常ケース）は make_reducer 経由で FilterChain の恩恵を受ける。
...
- テスト: 508/508 passed（core 464 + dx 44）
...
**フィルタ（pre/post/accum）は純粋関数であること。** 値の状態管理は `Ref[T]` で行い、
フィルタは入力から出力への変換のみを担う。この制約により、clone 時にフィルタの
クロージャ参照を安全に共有できる（alias / proxy プリミティブの前提条件）。
...
port.get()           // T? — 推奨: cobra-style アクセス
verbose.get()        // T?

// 階層ナビゲーション（ParseResult 経由）
result.child("serve")                    // ParseResult? — サブコマンド結果

// 非推奨（後方互換のため残存）
result.get(port)     // T? — parser.get(opt) に委譲。opt.get() を推奨
```
...
pub(all) enum OptKind {
  Flag        // Bool フラグ / カウンタ
  ValueOpt    // 値を取るオプション（string, int, append_* 等）
  Command     // サブコマンド
  Positional  // 位置パラメータ
  Rest        // 可変長位置パラメータ
} derive(Show, Eq)
...
OC/P の2フェーズで構成される（DR-034）:

- **OC Phase (Option/Command)**: ExactNode 走査 + 最長一致マッチ。greedy positional があればここで消費。マッチしなかった引数は unclaimed に蓄積
- **P Phase (Positional)**: force_unclaimed（`--` 以降）+ unclaimed を結合し、non-greedy positionals で前から順に消費。greedy positionals は OC で消費済みなのでスキップ
...
1. === OC Phase (Option/Command) ===
   メインループ（while pos < args.length）:
   全 ExactNode に対して try_reduce(args, pos) を呼ぶ:
   - Accept(consumed, commit) → 候補残留
   - Reject → 候補脱落
   - Error(e) → 即座に raise ParseError

   Accept した候補のうち consumed が最大のものを選ぶ:
   1a. 最大 consumed が複数（ambiguous） → raise ParseError
   1b. 最大 consumed が1つ → 勝者確定 → commit() + pos += consumed
   1c. Accept した候補が0:
       - greedy positional があれば OC で消費試行
       - 消費できなければ unclaimed に追加

2. === P Phase (Positional) ===
   force_unclaimed（-- 以降の args）+ unclaimed を結合
   greedy positionals をスキップ（OC で消費済み）
   non-greedy positionals で前から順に消費:
   - is_rest=false → 消費後に次の positional に進む
   - is_rest=true → 同じハンドラを繰り返し使用（stop_before で Reject → 次へ）
   全 positional 消費済みなら raise ParseError("unexpected argument")

3. post_hooks を順に実行（値変換・遅延バリデーション）
...
let serve_cmd = p.cmd(name="serve", setup=fn(child) {
  ...
  let host = child.string(name="host", default="localhost")
...
- `global=true` のノードは子パーサの nodes/global_nodes に伝播（宣言順序非依存）
...
pub(all) struct OptMeta {
  ...
  env_name : String        // 環境変数名（"PORT" 等）。空なら [env: X] を表示しない（DR-041 Phase 1）
...
2. ~~**kind の区別のユーザー API 表現**~~ → **解決済み**: コンビネータ（flag, string, int, cmd 等）で暗黙決定
...
- ~~**did you mean? サジェスト**~~ — Levenshtein 距離によるスペルミス候補提示【実装済み】
...
- ~~**custom** — ユーザー定義 reducer~~ → **実装済み**: `custom[T : Show]` — Show 制約により default_display を `T.to_string()` で自動導出。string, int は custom のシュガー
- **`get_or(T) -> T`** — `get().unwrap_or(default)` の糖衣。core の小さな DX 改善
- **file コンビネータ** — `string` / `int` と並ぶファイルパス特化コンビネータ。`implicit_value` との組み合わせで3値パターン（未指定=None / フラグのみ=デフォルトパス / 値指定=指定パス）を自然に表現。`default=file(arg="~/.config/xxx/xxx.toml")` のように、デフォルト値にもファイル存在チェック等のバリデーションを適用できる構想。clap で `num_args=0..=1` + サブコマンド衝突が起きる問題を、kuu は ExactNode 走査 + コンビネータ設計で構造的に回避
- ~~**alias / adjust プリミティブ (DR-037)**~~ → alias **実装済み**、deprecated **実装済み**（DR-040: alias + post_hook 方式）。残り: clone（型消去問題、Opt[T] setter 追加後）、adjust（同上）
...
string の `post` パラメータで FilterChain を post_hooks に登録する仕組み。
...
| 値オプション | `p.string(name=...)` / `p.int(name=...)` → `Opt[T]` | 実装済み |
| カウンタ | `p.count(name=...)` → `Opt[Int]` | 実装済み |
| 配列蓄積 | `p.append_string(name=...)` / `p.append_int(name=...)` → `Opt[Array[T]]` | 実装済み |
| サブコマンド | `p.cmd(name=..., setup=fn)` → `Opt[CmdResult]` / `p.sub(name=...)` → `Parser`（子パーサ直接返却） | 実装済み |
| 位置パラメータ | `p.positional(name=..., greedy?=false)` → `Opt[String]`。`greedy=true` で OC Phase 消費（DR-034） | 実装済み |
| 可変長引数 | `p.rest(name=..., stop_before?=[])` → `Opt[Array[String]]` | 実装済み |
| `--` (double dash) | `p.dashdash()` / `p.append_dashdash()` | 実装済み |
| choices | `p.string(choices=[...])` / `p.int(choices=[...])` | 実装済み |
| implicit_value | `p.string(implicit_value=...)` / `p.int(implicit_value=...)` | 実装済み |
| コンポジットノード | `make_or_node` — 最長一致で統合 | 実装済み |
| スコープグローバル | `global=true` パラメータ | 実装済み |
| ショートオプション | `shorts="pP"` パラメータ（複数 short 対応） | 実装済み |
| エイリアス | `aliases=["alias"]` パラメータ（内部で alias プリミティブに展開予定、DR-037） | 実装済み |
| alias | `p.alias(name, target)` — 汎用別名プリミティブ。値共有 + is_set 独立。チェーン alias 対応（DR-037） | 実装済み |
| adjust | `p.adjust(opt, before_pre=..., after_post=...)` — 振る舞い調整プリミティブ（DR-037） | 未実装 |
| deprecated | `p.deprecated(name, target, msg)` — alias + post_hook 方式。パース後に `deprecated_warnings()` で警告取得（DR-040） | 実装済み |
...
| 排他オプション | `parser.exclusive(opts)` | 実装済み |
| 最低1つ必須 | `parser.at_least_one(opts)` | 実装済み |
| require_cmd | `parser.require_cmd()` — サブコマンド必須制約（DR-026） | 実装済み |
| 繰り返しグループ | `group(name, tmpl)` — 雛形 clone | 未実装 |
| 条件付きオプション | `requires=[...]` | 未実装 |
| 環境変数連携 | `env`, `env_prefix`, `auto_env` | Phase 1 実装済み（メタデータ+ヘルプ表示、DR-041） |
...
src/
  ...
  dx/             # struct-first DX 層（Parseable trait + FieldRegistry + parse_into）
    ...
    options.mbt    # custom, custom_append, flag, string, int, count, append_string, append_int
...
- **型定義**: Opt[T], OptRef, Parser, ExactNode, PositionalEntry, AliasSource, TryResult, OptMeta, CmdResult, ParseResult, ParseResultKind, OptKind, Lazy[T], Variation, ReduceCtx[T], ReduceAction, FilterChain[A, B], Accumulator[T, U], ParseError, ParseErrorInfo
- **コンビネータ**: flag, string, int, count, append_string, append_int, custom[T : Show], custom_append[T], positional, rest, cmd, sub, dashdash, append_dashdash
...
- **テスト**: 464/464 passed（core パッケージ）

#### 計画外の追加実装（MVP 完了後）

- **choices**: string に choices パラメータ追加。make_choice_value_node + one_of バリデーション
- **implicit_value**: string/int に implicit_value パラメータ追加。make_or_node による composite ノード構築
- **stop_before**: rest/dashdash に stop_before パラメータ追加
- **short option**: flag/string/int/count/append_string/append_int に shorts パラメータ追加（`shorts="vV"` で複数 short 対応。OptMeta.shorts : Array[Char]）
- **aliases**: flag/string/int/count/append_string/append_int に aliases パラメータ追加
- **global**: flag/string/int/count/append_string/append_int に global パラメータ追加
- **hidden**: 全コンビネータに hidden パラメータ追加
- **post hooks**: string/int に post パラメータ（FilterChain）追加
...
- **default_fn**: flag/string/int に動的デフォルト値パラメータ追加。default より優先
- **名前重複検証**: validate_no_duplicate_names + expand_and_register での記録
- **Opt::is_set()**: 明示指定されたかどうかを判定
- **Opt::name**: オプション名フィールド
- **exclusive**: 排他オプション制約（post_hooks ベース）
- **require_cmd**: サブコマンド必須制約（post_hooks ベース）
- **custom[T : Show]**: 汎用値オプションコンビネータ。Show 制約で default_display を `T.to_string()` で自動導出。string, int は custom のシュガー
- **default_display 自動導出**: `default_display? : String? = None` → None 時は Show 制約で自動導出。int の「default=0 なら非表示」特殊処理は廃止
- **shorts 複数対応**: `short? : Char` → `shorts? : String = ""` に変更。`shorts="vV"` で `-v` と `-V` の両方が動作。OptMeta.shorts : Array[Char]
- **int に post/choices 追加**: string と同等の機能を custom へのパススルーで提供
...
- **struct-first DX 層 (DR-042)**: `src/dx/` に実装。Parseable trait + FieldRegistry + parse_into の2フェーズ方式。テスト44ケース
...
- `parser.required(opt)` — 単一オプション必須制約【実装済み】

Design rationale: 専用のフックパイプライン基盤は YAGNI。post_hooks で十分実現可能。

#### Step 5: defaults マルチソース

- 環境変数連携 Phase 1【実装済み】: `env` パラメータ（ヘルプ表示用メタデータのみ、DR-041）
- 環境変数連携 Phase 2（未実装）: 実際の環境変数読み取り、`env_prefix`, `auto_env`
- ValueSource トラッキング
- 各ソースごとに独立パース → 後勝ちマージ
...
基本的な排他制約は `parser.exclusive(opts)` で実装済み。`parser.at_least_one(opts)` も実装済み。`parser.required(opt)` で単一オプションの必須化も実装済み。
以下は当初の設計案と追加検討事項。
...
現在は `src/core/` に全機能を統合。ファイル分割で責務を分離。`src/dx/` に struct-first DX 層（DR-042）、`src/wasm/` に WASM bridge:
...
src/dx/
  types.mbt            # Parseable trait, FieldRegistry struct
  parse.mbt            # parse_into（register → apply_all 2フェーズ）
  registry.mbt         # flag, string, int, count, append_string, append_int（_ref 版含む）
  registry_pos.mbt     # positional, rest
  registry_cmd.mbt     # sub, require_cmd, set_description
  registry_custom.mbt  # custom[T], custom_append[T]（_ref 版含む）
  constraints.mbt      # exclusive, required, at_least_one
  dx_wbtest.mbt        # DX 層テスト（44ケース）
src/wasm/
