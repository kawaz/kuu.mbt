...
# DR-0008: ReduceCtx 統一 + フックアーキテクチャ

> **注記**: 本 DR の以下の設計は実装時に簡略化・更新されています:
> - **6フェーズパイプライン** (PreProcess→Resolve→Reduce→Validate→Finalize→Output): 実装では3段階 (parse_raw + consumed=0 finalize + post_hooks) に簡略化。詳細は本 DR 内の「実装注記」および DR-0034 (OC/P 2フェーズ分離) を参照
> - **Visibility 4段階** (Visible/Advanced/Deprecated/Hidden): 実装では `hidden: Bool` に簡略化
> - **ReduceCtx[T] 統一シグネチャ**: 採用・実装済み
...
- `docs/research/2026-03-02-error-message-survey.md` — 5パーサのエラーメッセージ調査
...
- DR-0007 (`docs/decisions/DR-0007-phase4-opts-enum-resultmap-design.md`) — 前回の設計決定（Opts enum + ResultMap）
