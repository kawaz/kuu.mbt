...
# DR-0045: ValCell / Accessor 分離 — Opt[T] から値のライフサイクルを独立させる

**ステータス: accepted (Phase 1-4 implemented)**

## 背景

DR-0037 で 3 直交プリミティブ（clone, link, adjust）を設計し、DR-0043 で `priv setter` を追加、
DR-0044 で v1 実装を完了した。しかし実装を進める中で、Opt[T] が「値の格納」と「パース振る舞い」の
...
DR-0044 で clone は v1 として alias のラッパーとして実装された:
...
DR-0037 の本来の設計では clone は「新 Ref、フィルタ参照共有」— つまり独立した値セルを持つ
...
DR-0044 の link 実装では、`setter` が cell 値のみを更新し `was_set` は変更しない
（DR-0043 の設計原則: 「setter は cell 値の直接書き換えのみ。was_set は更新しない」）。
...
これにより DR-0043 の「setter は was_set を更新しない」制約が解消され、
...
DR-0037 の本来の設計「新 Ref、フィルタ参照共有」が自然に実現される。
...
3. DR-0044 の「clone は alias のラッパー（v1）」制約を解消
...
- **DR-0037**: 3 直交プリミティブ（clone, link, adjust）の設計 — ValCell で clone が本来の姿に
- **DR-0043**: `priv setter` — ValCell / Accessor に発展的に吸収される
- **DR-0044**: clone/link/adjust v1 実装 — ValCell 導入で v2 へ進化
- **DR-0027**: core 純粋関数主義 — ValCell は純粋な値コンテナ、副作用なし
- **DR-0042**: struct-first dx 層 — dx は引き続き Opt の public API のみ使用
