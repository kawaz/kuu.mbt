...
# DR-0049: サロゲートペア対応（grapheme 導入）

## 概要

MoonBit の String は UTF-16 内部表現を採用しており、`length()` や `str[i]` は UTF-16 コードユニット単位で動作する。kuu の short combining と levenshtein 距離計算において、Supplementary Plane 文字（U+10000 以上、例: 絵文字）が正しく処理されない問題を `kawaz/grapheme` ライブラリの導入で解決する。
...
| レイヤー | 問題 | 対応方法 | 対応時期 |
|----------|------|----------|----------|
| L1: UTF-16 encoding | `length()` + `str[i]` がコードユニット単位 | grapheme 導入 | 本 DR |
| L2: Grapheme cluster | 合成絵文字（🇯🇵 👨‍👩‍👧‍👦 等）が複数コードポイント | grapheme UAX #29 対応済み → levenshtein/short combining は自動対応、shorts 登録は未対応 | 一部完了 |
| L3: Display width | 全角/半角の表示幅 | `rami3l/unicodewidth` | 必要時 |

本 DR では L1 の問題を解決する。修正手段として `char_length()` / `to_array()` ではなく `grapheme` の `graphemes()` を使うことで、将来の L2 対応時に必要なコード変更量を最小化する（ただし shorts の型変更等、別途対応が必要な箇所は残る。詳細は末尾「L2 対応時の追加変更」を参照）。

## 方針: grapheme による統一的な文字分割

### なぜ char_length() / to_array() ではなく grapheme か
...
DR-0001 の9箇所には含まれていないが、念のため分析する。
...
`path` は `moon.mod.json` からの相対パス。kuu.mbt の `main/` から grapheme の `main/` へは `../../grapheme/main`（`main → kuu.mbt → kawaz → grapheme → main`）。

ただし mooncakes.io に公開する場合はバージョン指定に切り替える必要がある:

```json
"kawaz/grapheme": "0.1.0"
```

公開前に grapheme 自体も mooncakes.io に publish しておく必要がある。
...
```
import(
  "kawaz/grapheme/src" as grapheme
)
```

→ `@grapheme.graphemes(s)` で呼び出し可能。
...
4. 修正コードで `@grapheme.graphemes()` を使用
...
### grapheme の UAX #29 対応状況

grapheme は UAX #29 (Unicode 16.0.0) 全対応を完了。公式テスト全1,093ケース合格済み。
...
本 DR で修正した箇所（levenshtein、short combining 展開）は grapheme の分割粒度に依存するだけなので、UAX #29 対応により **L2 も自動的に解決済み**:
