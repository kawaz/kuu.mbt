...
# DR-0044: clone / link / adjust コンビネータ実装

> **Note**: DR-0045 で大幅に進化。clone は alias ラッパー(v1)から独立 cell の save/restore パターンに変更。link に propagate_set~ パラメータ追加。alias の commit は Accessor.set_commit 経由に統合。

## 背景

DR-0037 で設計した 3 直交プリミティブ（clone, link, adjust）と、
DR-0043 で追加した `priv setter` により、実装の準備が整った。
...
**DR-0037 との差分**: DR-0037 は link を「Ref 共有（ポインタ付け替え）」として定義しているが、
...
DR-0037 は clone を「新 Ref、フィルタ参照共有」と定義しているが、
