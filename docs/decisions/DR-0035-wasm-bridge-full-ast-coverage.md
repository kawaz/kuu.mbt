...
# DR-0035: WASM bridge 拡張 — JSON 表現可能な全機能の対応
...
DR-0033 で WASM bridge の制限事項を整理し、一部の機能を「ホスト言語側で対処が自然」と分類した。しかし DR-0030 の opt AST ポータビリティの思想に基づけば、JSON で表現可能なものは全て bridge 側で対応すべき。
...
これらは「bridge の制約」ではなく「DX ラッパーの責務」。ホスト言語のDXラッパーが「パースは WASM、変換/検証はホスト」の2段階パイプラインを組むことで、ユーザーからはシームレスに見える。プリセット post（trim, non_empty, in_range 等）は bridge 側で処理し、任意クロージャはホスト側で処理する自然な分担となる（DR-0027 の多言語DX戦略の具体的実現形態）。
...
- DR-0033: WASM bridge 制限事項（この DR で方針を更新）
