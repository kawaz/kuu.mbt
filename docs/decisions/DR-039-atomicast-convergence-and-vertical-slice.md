...
> 由来: kawaz × Claude のリモートセッション議論 (2026-06)。トップダウン AST とボトムアップ実装の合流地点を確定。
...
- `parse_raw` は `Array[ExactNode]` だけを舐め、`commit : () -> Unit` という型を持たない普遍クロージャで解釈を確定 → AtomicAST の {exact/or/seq/primitive} ＋ 最長一致ループと構造同型。
...
2. 評価器         bounded path-search (DR-038)。完全経路の数を数え 0/1/2+ で結末。
                  TryResult/Reject/Error を再利用、parse_raw の greedy-commit ループは置換。
...
AtomicAST はパーサの入力言語そのもので実装と密結合。JSON Schema を先に固めるのは罠 (CONTEXT 論点 A/K)。よって walking skeleton で共設計:
...
## MoonBit 現状の含意 (2026-06)

- v0.9 まで確認。trait object `&Trait` 有り。**generic trait `trait Foo[T]` と custom derive は未提供**。polymorphic method は最近 (v0.9 前後)。
...
- ボトムアップ kuu: `src/core/parse.mbt`、`src/core/nodes.mbt`、`src/core/types.mbt` (TryResult/ExactNode)、DR-042 (struct-first DX、derive 待ち)
