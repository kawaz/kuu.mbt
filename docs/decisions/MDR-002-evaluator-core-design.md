...
- 実装後、slice `poc/phase16_wbtest.mbt` 相当の fixture (`seq a b c` → `{seq:{xs:[a,b]}, tail:c}`) で破れ 1 の解消を実証する (§1.5 受け入れ条件)。
- §2.6 の先食い判定の潜在穴 (値なし greedy トリガが入力末尾で沈黙 → positional に raw 食い) を突く fixture を参照実装側で明示化し、Pending 統一で塞がれることを検証する。
