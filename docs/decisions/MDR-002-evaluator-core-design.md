...
- **greedy エントリの「1 回」制約 (DR-041 §4) は未実装**: 同一 entry が無制限に再発火する。座席は評価器でなく installer / count registry の見込み (AtomicAST の greedy 面に repeat 有無の区別が無い)。docs/issue/2026-07-08-greedy-once-and-idxrepeat-premises.md で追跡。
- **IdxRepeat は bounded-head 前提 (下流 backtrack なし)**: permissive head + 後続 positional では DR-043 の字義 (後退) と乖離する。前提を強制する検査は無く、installer が bounded head にのみ使う規約 (node.mbt design rationale) — 同 issue で明文化を追跡。
- **tried_triggers の失敗位置対応**: DR-053 §4 の「失敗位置で試行された綴り」に対し現実装は root スコープ固定 (Or 対応は済み)。素材の設計が要るため同 issue で追跡。

- **complete 実装 (出口層) はモード分岐を要する見込み**: §2.4 の「収集ポリシーを足すだけ」は、(a) 取り分選好の刈り込みが負け枝の Pending を捨てる、(b) `drop_optional_missing` が repeat 続き候補を落とす、(c) `consume_head` の枯渇が Pending でなく Held である — の 3 点と衝突する (設計レビューで確定、DR-060 §1 自身のヘッジ「専用走査が正しい形になりうる」が想定していた事態)。complete 実装時に「walk へ complete モード (刈り込み無効 + 枯渇の Pending 化) を通す」か「DR-060 のヘッジ通り別走査」かを決め、本 DR の §2.4/§2.7 を改訂する。
