...
### KTop での residual エラー自然発生

slice がトップレベルで合成していた残余トークンエラー (`unexpected_token`) は、`KTop` が `pos != toks.length()` で Held を返すことで各 dead-end 分岐から自然に発生する。**異なる argv_pos の unexpected_token が複数 errors に乗りうる** (DR-053 §2 全保持により忠実)。conformance fixture が「1 個の合成エラー」前提なら fixture 実食時に期待値の突き合わせが要る。
