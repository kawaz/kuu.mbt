...
# DR-0053: プリミティブ分解 — name/exact/serial/or によるオプション構造の一般化
...
option の中に入るプリミティブは2種類に分かれる:

#### アクションプリミティブ（consumed=0、値を外部から取らない）

名前マッチのみで値が確定する。次の引数を消費しない。

| プリミティブ | 型 | consumed 時の振る舞い |
|---|---|---|
| `flag()` | `Bool` | `cell = true`（存在で真） |
| `count()` | `Int` | `cell += 1`（出現回数カウント） |

```
option(name="verbose") { flag() }    // --verbose → true
option(name="verbose") { count() }   // --verbose --verbose → 2
```

#### 値消費プリミティブ（consumed=1、次の引数を値として取得）

FilterChain ベースで次の引数を型変換して消費する。
...
| `boolean` | `Bool` | `Filter::parse_bool`。true/t/on/1, false/f/off/0。flag と違い値を取る |

```
option(name="host") { string() }     // --host example.com → "example.com"
option(name="port") { int() }        // --port 8080 → 8080
option(name="debug") { boolean() }   // --debug true → true（flag と違い値が必須）
```
...
| | flag | boolean |
|---|---|---|
| 値の取得 | なし（存在で true） | あり（true/false/on/off/1/0） |
| consumed | 0 | 1 |
| `--debug` のみ | `true` | エラー（値が必要） |
| `--debug true` | エラー（true は次のオプション扱い） | `true` |
| 用途 | `--verbose`, `--dry-run` | `--color true`, `--ssl false` |
...
DR-0053 のプリミティブ分解では `or(...)` や `serial(...)` のようなコンテナに name は不要。`name` コンビネータが独立しているため、名前付与は明示的に行う。Sugar 層は引き続き name 必須でよい。
...
現在の `link`（post_hook で一方向コピー）では表現できないパターンがある。例:

```
--time-range <SINCE:TimeSpec> <UNTIL:TimeSpec> | <RANGE:TimeRange>
--since      <SINCE:TimeSpec> | <RANGE:TimeRange>
--until      <UNTIL:TimeSpec> | <RANGE:TimeRange>
```

ここでは3つの名前付き値セル（SINCE, UNTIL, RANGE）が option を跨いで共有される:

```
SINCE ← --since の TimeSpec, --time-range の TimeSpec(1番目), RANGE.since
UNTIL ← --until の TimeSpec, --time-range の TimeSpec(2番目), RANGE.until
RANGE ← --since の TimeRange, --until の TimeRange, --time-range の TimeRange
```

さらに RANGE は構造体であり、RANGE.since → SINCE、RANGE.until → UNTIL という**射影リンク**が存在する。どの option からどの形式で入力されても、最終的に SINCE/UNTIL/RANGE の値が整合する必要がある。

### 値記述子パターン: ユーザーが事前にセルを作って渡す

最もシンプルな解法: ユーザーが値記述子（ValCell を内包したプリミティブ）を先に作り、option に渡す。

#### シンプルな例: flag + boolean の共存

```
let b = bool()
option(name="flag") { or([flag(link=b), b]) }
// --flag     → flag が勝ち(consumed=0)、b に true を書く
// --flag true → b が勝ち(consumed=1)、b に true を書く
// --flag false → b が勝ち(consumed=1)、b に false を書く
```
...
```
let tr = timerange()
let since = or(tr, tr.since())  // TimeRange 全体 or since 射影
let until = or(tr, tr.until())  // TimeRange 全体 or until 射影

option(name="time-range", tr)
option(name="since", since)
option(name="until", until)
```

- `tr.since()` は「親 TimeRange の since フィールドに射影した Opt[TimeSpec]」を返す。型の作者が提供するメソッド
- `or` は**パターン選択コンビネータ**であり、値の型を統一しない。どのブランチが勝っても、そのブランチのセルに値が書かれる
- 射影リンクにより、TimeRange が書かれれば since/until も自動更新される

#### 設計上のポイント

- **値記述子の配線**: `bool()` 等が返す記述子は ValCell を内包。`option()` に渡された時点で parser に登録・配線される。set~ のアクセサは option が繋ぐ
- **or は型非依存**: or は最長一致でブランチを選ぶだけ。各ブランチは独立したセルを持ち、値の流れはセルと射影が担う
- **射影は型の責務**: `tr.since()` のような射影メソッドは kuu core ではなく型の作者が定義。core は射影の仕組みを知る必要がない
- **register 時の課題**: 値記述子を parser にどう登録するか（特に or の中に異なる型の記述子が混在する場合）は実装時に検討が必要

### 現在の link との関係

現在の `link(target, source~)` は post_hook ベースの一方向コピー。値記述子パターンが実現すれば、link はこのパターンの低レベル実装として位置づけられる。
