...
# DR-0020: 汎用 or コンビネータ + Initial[T] — choices/implicit_value の再設計

## 背景

DR-0019 で optional value option（choices + implicit_value）を実装したが、`choices: Array[String]` が String 固定で汎用性がないことが判明。実装は撤回し、設計からやり直す。
...
- `uwptxwyy` feat: choices バリデーション + implicit_value (optional value option) 実装 (DR-0019)
- `xmqslovn` feat: implicit_value 全コンビネータ展開 + post フィルタ + = マーカーヘルプ (DR-0019)
...
- **sub-parser の配列としての choices**: `make_or_node` が複数の子 ExactNode を受け取り、最長一致で統合。DR-0019 の `Array[String]` 固定から脱却
