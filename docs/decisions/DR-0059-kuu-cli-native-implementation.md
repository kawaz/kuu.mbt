...
kuu の多言語展開において、WASM bridge (Layer 2a) はインプロセスで高速だが、ワンバイナリ化できない（ランタイム依存）という弱点がある。DR-0047 で kuu-cli embed パターンを構想し、DR-0057 で独立コマンドとしてのビジョンを描いた。本 DR はその実装に関する技術的判断を記録する。
...
{
  "opts": [
    { "kind": "flag", "name": "verbose", "description": "..." },
    { "kind": "string", "name": "port", "default": "8080", ... }
  ],
  "subcommands": [...],
  "constraints": [...]
}
...
- clone した opt は link/adjust/deprecated の target にできない（WASM bridge と同一の制限）

## 関連 DR

- DR-0027: core 純粋関数主義 + 多言語 DX レイヤー構想
- DR-0036: KuuCore 統一低レベル API（4層アーキテクチャ）
- DR-0047: kuu-cli embed+extract+exec パターン
- DR-0057: kuu-cli 独立コマンドとしてのビジョン
- DR-0058: 構造化エラー表示（エラー JSON 出力に影響）
