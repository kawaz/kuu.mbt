...
- **dd installer**: `type: "dd"` を回収し、greedy 面に matcher を持たない exact 衛星 (トリガ兼消費者) を追加。発火後は positional 継続を内部消費として引き継ぐ (sever は DR-041 §4 の内部一体則から導出、不変則⑤)
...
| `dd` | `type: "dd"` | greedy 面の exact 衛星 (matcher なし)。発火後は継続を内部消費として引き継ぐ |
...
- **installer の類型は「何を植えるか」の違い**: long / short は衛星 + matcher、dd は衛星のみ (matcher なし)、env は席宣言のみ。いずれも同じ 3 役 (回収・植え付け・能力提供) の部分集合
