...
# DR-0057: kuu-cli — 言語非依存の独立コマンドとしての kuu
...
4. kuu-cli embed パターン (DR-0047) で単一バイナリとしての配布も実証済み
...
| **kuu-cli embed** (DR-0047) | ランタイムなし単一バイナリ配布 | PoC 済み |
...
- DR-0029: 言語境界を越える Serialize/Deserialize 設計構想
- DR-0030: opt AST の言語間ポータビリティ
- DR-0047: kuu-cli embed+extract+exec パターン
- DR-0046: LLVM backend ネイティブ FFI 実現可能性調査（不可の結論 → fork で解決）
