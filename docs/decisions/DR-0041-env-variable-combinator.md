...
- **値取得** は `getenv()` 等の副作用を伴う。コア純粋関数主義（DR-0027）に従い注入可能にする
...
### 5. WASM bridge（DR-0035）への反映

- JSON schema に `env` フィールドを追加
- EnvSource は WASM 側（ホスト言語）から注入する形になる
- opt AST ポータビリティ（DR-0030）にも影響: env 定義を JSON export に含める
