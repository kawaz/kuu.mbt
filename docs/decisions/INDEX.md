...
- [MDR-006](MDR-006-package-separation.md): engine/builtins/kuu の package 分離 — DR-110 (kawaz/kuu) の MoonBit 実装設計。物理 3 package (src/{engine,builtins,kuu}、import 方向を moon.pkg で強制、MDR-002 §3 を package 依存に昇格)、可視性三分類 (pub(all) 玄関 / pub(open) trait + read-only pub の extension 契約 / priv 内部 — 「pub はフィールド読み・variant match 可、構築のみ禁止」の実機訂正込み)、契約シグネチャ (NodeExt/MatcherExt/TypeExt/InstallerExt + Resume 関数型で Cont 隠蔽、fingerprint でなく encode:Json 一本化、trait default method 活用)、テスト配置 (既存 wbtest 10 本の行き先マップ、engine は合成住人で検証)、実施計画 M1〜M5 (各里程標末 green push、conformance runner は M1=kuu 内 wbtest → M5 blackbox 最終化の段階昇格)、kuu-cli 破壊 3 波 (M1 import path / M3 Cand.ty String 化 / M5 sentinel・内部走査の玄関吸収) と lockstep 移行パス。MDR-005 §4 (分割不採用) を supersede
