...
- 破れ 1 は評価器の再設計で必ず解消する。受け入れテストは slice `poc/phase16_wbtest.mbt` の凍結 fail。
- 破れ 2 は評価器の設計スケッチを両案 (分離恒久化 vs Branch に Pending 追加) で描いてから決める (現時点未決)。
...
- MoonBit 予約語との衝突 (`alias` / `export` / `inherit`) を避ける命名規約は、評価器の設計スケッチと同時に決める (現時点未決)。
...
1. **破れ 2 の Pending 表現**: 分離恒久化 vs Branch に Pending 追加。評価器の設計スケッチを両案で描いてから決める。
2. **予約語衝突の命名規約**: `alias` / `export` / `inherit` が MoonBit 予約語。評価器設計スケッチと同時に決める。
