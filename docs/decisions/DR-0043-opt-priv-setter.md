...
# DR-0043: Opt[T] に priv setter を追加

> **Note**: DR-0045 で ValCell/Accessor に発展的統合された。setter は Accessor.set / set_value / set_commit に吸収され、Opt[T] の priv setter フィールドは priv accessor: Accessor[T] に置換された。

## 背景

DR-0040 で clone / full adjust の実装が「Opt[T] に setter がないため保留」とされていた。
...
3. DR-0040 で保留の clone / adjust 解禁を検証（別 DR で扱う）
