...
DR-037 で設計された adjust の 6 パラメータ（before/after × pre/post/accum）が DR-044 で「v1 は after_post のみ」に縮小され、その結果 `append_string`, `append_int`, `append_float`, `append_dashdash`, `custom_append`（現 `append`）が独立実装として増殖した。
...
**因果関係**: DR-044 で adjust が after_post のみに縮小された結果、蓄積戦略の変更（append）を adjust で表現できなくなり、`custom_append`（現 `append`）, `append_string`, `append_int`, `append_float`, `append_dashdash` が独立実装として増殖した。元の 6 パラメータ設計が実装されていればこれらは全て不要だった。
