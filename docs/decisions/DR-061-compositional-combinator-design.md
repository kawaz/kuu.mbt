...
# DR-061: コンビネータの合成的分解設計

## 背景

DR-037 で設計された adjust の 6 パラメータ（before/after × pre/post/accum）が DR-044 で「v1 は after_post のみ」に縮小され、その結果 `append_string`, `append_int`, `append_float`, `append_dashdash`, `custom_append` が独立実装として増殖した。
...
DR-037 では pre/post/accum の 3 フェーズとしたが、append の実態は after_post + 外部配列への push であり、独立した accum フェーズなしで実装できる可能性がある。
...
DR-037 の元設計では 3 フェーズ × before/after の 6 パラメータ:
...
| 現状 | 合成表現 |
|---|---|
| `custom_append[T]` | `append(custom[T](...))` |
...
| DR | 内容 |
|---|---|
| DR-037 | adjust 6 パラメータの元設計 |
| DR-044 | 「v1 は after_post のみ」に縮小 — **設計の勝手な変更** |
| DR-045 | after_post のまま Accessor 統合 |
| DR-053 | プリミティブ分解の構想（append[T] 列挙あり） |
| DR-060 | example レビューで append 系独立実装の問題が顕在化 |
| DR-061 | 本 DR。元設計に立ち返り合成モデルを明文化 |

**因果関係**: DR-044 で adjust が after_post のみに縮小された結果、蓄積戦略の変更（append）を adjust で表現できなくなり、`custom_append`, `append_string`, `append_int`, `append_float`, `append_dashdash` が独立実装として増殖した。元の 6 パラメータ設計が実装されていればこれらは全て不要だった。
