...
dd マーカー (`--`) も greedy 面の住人 (matcher を持たない exact 衛星、下記適用節) なので、この規則がそのまま「寛容な string positional が `--` を値として食う」枝を抑止する。dd を positional 面のマーカーとして置くと `prog -a -- -b` が常に ambiguous になることが垂直スライス PoC で実測されており、dd の greedy 面所属はその帰結でもある。
...
### dashdash (`--`) — matcher を持たない greedy トリガ兼消費者
...
AtomicAST に専用ノード型・状態フィールドは不要。「継続を内部として引き継ぐ」の実装表現 (継続参照の構造 / 経路局所の sever フラグ + 純マーカー) は観測挙動が同一であれば自由 (垂直スライス PoC は sever フラグ形で同一挙動を確認済み)。**dd はトークン境界を再解釈しないため matcher を持たない** (greedy 面には matcher なしの exact 衛星として座る)。
