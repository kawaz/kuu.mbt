...
dd マーカー (`--`) も greedy 面の住人 (matcher は素の exact 一致、下記適用節) なので、この規則がそのまま「寛容な string positional が `--` を値として食う」枝を抑止する。dd を positional 面のマーカーとして置くと `prog -a -- -b` が常に ambiguous になることが垂直スライス PoC で実測されており、dd の greedy 面所属はその帰結でもある。
