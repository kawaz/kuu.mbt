...
# DR-0007: Opts enum + ResultMap + immutable Opt + greedy/never 設計（2026-03-01）

> **注記**: 本 DR で設計した以下の概念は後続 DR で上書きされています:
> - **Opts enum** (`Node(&ErasedNode) | Array | Map`): DR-0012 で `ExactNode` ベースの直接保持形式に置換
> - **ResultMap** (`Map[Int, 型消去値]`): DR-0013 で `ParseResult` + `Ref[T]` による型安全アクセスに置換
> 本 DR は設計の論理的基礎として参照価値がありますが、現在の実装は DR-0012, DR-0013 を参照してください。
