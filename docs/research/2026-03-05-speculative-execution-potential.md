...
### バリデーション設計の議論で投機実行モデルの汎用性が浮上

Validate フェーズの設計議論で、ユーザーが「即時型」と「遅延型」の2種のバリデータを提案。この議論の中で、kuu の投機実行 + 最長一致モデルが単なるショートオプション結合以上の汎用性を持つことが明確になった。

#### ユーザー発言

> 次のトークン文字列を見た瞬間分かるものintとかstringとかregexとかのタイプ。
> もう一つは評価遅延タイプのバリデータ。これは排他とか少なくとも一つとか最大何個とかそういう他オプションを知らないと判定できないものにつかう。遅延タイプバリデーションは最初の走査時点ではvalidとして一旦仮食いする。（中略）そして全てのフォークした並行世界の最後まで全引数の消化が終わった時点で（中略）遅延バリデーションを行う

> 実はコレはさらに発展する話なんだけど、まずはバリデーションだけでこのパターンが有効か評価してほしい。

#### 評価結果（AI分析）

現モデル（ExactNode = 名前完全一致）では同率タイが起きないため、フォーク機構は発動しない。ただし:

1. **即時/遅延の分類は正しい**: 単一トークンで判定可能なもの（型チェック）と、パース全体の結果が必要なもの（制約チェック）の2種
2. **現モデルでは遅延バリデーションは post-parse チェックに縮退**: ExactNode が名前完全一致なので、ある引数位置で複数候補が同率 Accept することがない
3. **パターン自体は将来拡張に対して正しい**: 名前完全一致以外のマッチング（positional の型推論等）が入ると同率タイが起き、フォーク機構が必要になる

この評価を通じて、投機実行モデルの対応可能なパターン（ショート結合、型情報分解、数値引数等）の整理と文書化に至った。
