...
| **W2-5** (完了 2026-08-02) | **産出者を通す + 乖離検査**。DR-126 §4 の乖離検査を**射影で null 補形する前の生出力に対し** (DR-130 §4.1) — (a) 宣言外キー / (b) フィールド type の `out` 不一致 / 重複キー = Error、(c) 宣言済みキー不在 = 正常。sources の構造分解 (DR-127 §6 / DR-122 §3)。`TypeExt::parse_token` は `fn parse_token(Self, String) -> Result[Value, TypeParseFail]` のまま複合を返せるので signature を触らない | 達成: 656 tests / `ran_cases=888 mismatches=0`。`ResultValue` は**非統合**とし、**射影後の `Scalar` は複合を保持しない** canonical 化規則を規範として置く。**spec fixture (6) は本段では立たない** — record を名乗る builtin type factory が spec に無く、複合を産む住人が conformance 面に居ない (§3) | 実績: 中。結果は `docs/findings/2026-08-02-w2-5-producer-and-divergence.md` が正本 | opus5-worker-medium |
| **W2-6** (完了 2026-08-02) | **値空間残余の静的解決** (DR-127 §2.2 遷移表)。`resolve_link_path` の `Residual(leaf, from)` を value_type ごとの降下 (`classify_value_residual`) へ置換。record = フィールド当たり判定 (外れは `absent-ref`)、array = `[int]` 構造静的続行 / `.name` は `absent-ref`、map・value = 以降全部実行時 (静的検査を通過、routing は W2-7 まで不活性)、primitive = `absent-ref` (現状維持)、union = 含有 variant 1 つ以上 + 終端パーサ実体の一致検査 (不一致 = `invalid-range`)。accumulator セルへの値空間パスは `Unsupported` (§6.1)。cell fn `empty` の target 型検査 (REFERENCE §7.3、issue 2026-08-02-cell-fn-empty-target-type-check-missing を close) も本段で実装 | 達成: 661 tests / `ran_cases=889 mismatches=0`。**accumulator `Unsupported` の spec fixture は書けない** — `unsupported` は fixture kind enum に無い (CONFORMANCE の規定)。empty × scalar = `invalid-range` の fixture は追加済み。判断の根拠は `docs/findings/2026-08-02-w2-6-value-space-static-resolution.md` が正本 | 実績: 中 | fable5-worker-high |
| **W2-7** (完了 2026-08-02) | **vivify + 座への操作語彙**。FieldType 終端 (record フィールド) の residual routing を活性化 (`Binding.value_residual` + `LinkRoute`)。器 `{}` の auto-vivify (record 段まで、`set` 専用、DR-127 §3) と時系列 5 行を W2-4 共有 fold の `set_at_declared` 1 口で実装。operand はフィールド側 type で読む (§3.2、long/eq/short/positional の 4 入口とも lowering の型差し替え)。Sentinel (default/empty) Reject / 空座への `ctx.old` fn Reject (FnCtx の old 参照追跡で判定) / `set(null)` = 座を null へ戻す (DR-131 §2b/§7)。record null 補形 (DR-130 §4.1) を `build_result` + `align_sources_to_result` で実装 (issue `record-null-fill` close)。resident output contract は fn 戻り値分を消化 (残余は descriptor io_type 席の ABI 拡張待ち) | 達成: 682 tests / `ran_cases=889 mismatches=0`。Reject は W2-8 まで resolve 相 Err (単枝で §4 と同観測)。ElementShape / RuntimeResolved 終端は不活性のまま (W2-8 の実行時解決が持ち主)。**spec fixture (2)(5) は書けない** — record 産出 builtin 不在、DR-132 (fixture/* 仮想型) の実装窓で移送。正本は `docs/findings/2026-08-02-w2-7-vivify-and-seat-operations.md` | 実績: 高 (だが W2-4 の口の設計が時系列を吸収し、専用機構は不要だった) | fable5-worker-high |
| **W2-8** (完了 2026-08-02) | **§4.2 の枝ローカル効果列 fold** (W2-0 の spike を本実装へ)。枝落としの面は **Held 等価** (W2-0 findings の第 3 案) — 合成 ParseError (args_pos = 当該 binding の `at_pos`、無ければ消費位置、§6 の統括裁定 2) が既存の Held 経路 (max_reach / push_error / faildef) にそのまま乗る。実行時解決 (array index の現在長・負 index・範囲外 / map キー / `value`) が枝 Reject になり他解釈が勝つ。共有 fold は `src/internal/fold/` へ移設 (第 4 の fold は作らず、eval の committed 判定とは明示分離 — W2-4 §5.1 の裁定)。ElementShape / RuntimeResolved の routing も活性化 (operand は entry 自身の型のまま — §3.2 のフィールド type 差し替えは record フィールド終端固有) | 達成: 694 tests / `ran_cases=889 mismatches=0`。wbtest: 兄弟枝が勝つ / 全枝落ち + 原因合成 / map キー / 負 index 現在長解決 / 範囲外 args_pos 帰属 / scope 配下。**spec fixture (4) と (3) の実行時分は DR-132 実装窓 (#136) で移送** (record 産出 builtin 不在)。正本は `docs/findings/2026-08-02-w2-8-branch-local-fold.md` | 実績: 中 (W2-0 の形どおり追随、案 (b) 不要) | fable5-worker-high |
| **W2-9** (完了 2026-08-02) | **観測面の仕上げ**。effects `path` の値空間 index segment を解決済み非負値へ (`CellSeats::resolved_residual`、§6 の統括裁定 3)。sources の座 re-tag (部分書きした座だけ `link`、他は産出発火のタグ) — 配達は resolve 相 carrier の `Binding.source_shadow` (optional、parse 相は常に None = DR-038 効果 identity 不干渉)。実行一本化 (issue `cell-fn-multi-fire-unify`) は **FiringRecord 設計を確定して後続窓へ** (env/config 供給は parse 入口の optional Supplies で閉じる方向) | 達成: 702 tests / `ran_cases=889 mismatches=0`。時系列行 4 = `{since=cli,until=link}`、負 index = `effects=arr,arr[2]` + `sources=[cli,cli,link]` を wbtest pin。**spec fixture (6)(7) 値空間分は書ける状態** — DR-132 実装窓 (#136) で移送。正本は `docs/findings/2026-08-02-w2-9-observation-finish.md` | 実績: 中 | fable5-worker-high |
