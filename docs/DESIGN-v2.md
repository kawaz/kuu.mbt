...
投機実行と最適解釈で曖昧さを構造的に解決する CLI パーサエンジン。MoonBit 製、多言語基盤としても設計されている。

核心: 各コンビネータが消費可能な範囲を投機的に試し、最も妥当な解釈を採用する。全 OptKind を `initial + ReduceCtx reducer` で統一し、消費ループは型を知らず、名前解決も持たない。全ての複雑さはコンビネータ層で事前展開される。

### 位置づけ

- **PEG（ordered choice）との違い**: PEG は最初にマッチした候補を採用する。kuu は全候補を投機実行して最適な解釈を選ぶ。順序非依存であり、曖昧さ（同率候補）も検出できる
...
| `docs/decision-records/` | 個別の設計判断記録（DR-001〜DR-047） |
