...
MoonBit 製の CLI 引数パースエンジン。多言語基盤としても設計されている。

核心: **全 OptKind を `initial + ReduceCtx reducer` で統一**し、ExactNode の**投機実行 + 最長一致**で消費する。消費ループは型を知らず、名前解決も持たない。全ての複雑さはコンビネータ層で ExactNode に事前展開される。

### 位置づけ

kuu は「CLIパーサ」であると同時に、汎用パーサコンビネータと同等の合成能力を CLI ドメインに特化した形で実現している:

- **PEG（ordered choice）との違い**: PEG は最初にマッチした候補を採用する。kuu は全候補を投機実行して最長 consumed を選ぶ。順序非依存であり、曖昧さ（同率候補）も検出できる
- **一般的な CLI パーサ（clap, cobra 等）との違い**: 一般的パーサは「名前を引いてから値を処理」する2ステップ方式。kuu は各ノードが自己判定する。これにより `-abc` 結合分解、`--color`/`--color=always` の共存、choices バリデーションが同一アルゴリズムで解決される
- **モナディックパーサコンビネータとの違い**: oni-comb-rs 等は `Parser<A>` の `flat_map`/`or` でモナディック合成する。kuu の FilterChain は同じ Kleisli 合成（`then`）を持ち、Accumulator で変換と蓄積を分離している。合成能力は同等だが、CLI ドメインでは ExactNode の完全一致名マッチにより、バックトラッキングの複雑さが構造的に消える

---

## コアの問題と解法

### 問題: heterogeneous collection without reflection
...
### 解法: クロージャによる型消去 + Ref[T] 共有

各コンビネータ内で `Ref[T]` を生成し、**2つのクロージャが同じ `Ref[T]` をキャプチャ**する:

```
コンビネータ（例: string_opt）内部:
  let cell : Ref[String] = { val: default }

  ExactNode.try_reduce クロージャ ─→ cell を書き込み（型消去ビュー: T を知らない）
  Opt[T].getter クロージャ ────────→ cell を読み出し（型ありビュー: T として返す）
```

- **型消去ビュー**: ExactNode の `try_reduce : (Array[String], Int) -> TryResult` は型パラメータを持たない。commit クロージャが `cell.val = parsed_value` を行うが、この `cell` は `Ref[String]` であることを ExactNode 自体は知らない
- **型ありビュー**: `Opt[String].getter : () -> String` が同じ `cell` をキャプチャし、`String` として読み出す
...
Core:        ExactNode (try_reduce) + OC/P 消費ループ + プリミティブ（clone, link, adjust）
```
...
ユーザーは `opt.get() -> T?` で値を取得。パース前なら `None`、パース後なら `Some(T)`。

Opt は薄い参照型。initial / reducer / meta はコンビネータ内のクロージャにキャプチャされ、Opt 自体には含まれない。getter がキャプチャする `Ref[T]` とコンビネータ内の ExactNode が同じ `Ref[T]` を共有するため、型安全性は静的に保証される。
...
「片の型 U」（1回の入力から変換された値）と「累積型 T」（最終的な結果の型）を分離する:
...
**フィルタ（pre/post/accum）は純粋関数であること。** 値の状態管理は `Ref[T]` で行い、フィルタは入力から出力への変換のみを担う。この制約により、clone 時にフィルタのクロージャ参照を安全に共有できる（直交プリミティブの前提条件）。
...
| `Reset(p)` | `cell=default, was_set=true` | デフォルト値に戻す（明示指定扱い） |
| `Unset(p)` | `cell=default, was_set=false` | 完全に未指定状態に戻す |

全コンビネータで `variations=[]` がデフォルト（自動で --no- を生やさない）。Sugar パラメータ（`variation_toggle?` 等）で明示指定する。

### 実装済みの合成コンビネータ

- **alias** — `p.alias(name, target)`: 値共有 + is_set 独立の別名。チェーン alias 対応（alias の alias が root の was_set まで伝搬）。global alias は子パーサに伝播
...
### Opt[T] setter 追加による進化（進行中）

現状の Opt[T] は getter のみで読み取り専用。clone/adjust は Opt[T] に setter を追加することで実装可能になる:

- **setter/getter クロージャペア**で値アクセスを抽象化。Ref[T] の存在は完全にクロージャの内側に隠蔽
- clone: 新しい setter/getter ペア（新 Ref をキャプチャ）を持つ Opt を生成
- link: target の setter/getter をそのまま使う（同じ Ref を掴んだクロージャを共有）
- adjust: setter をラップ（`fn(v) { original_setter(filter(v)) }`）

ValRef のような中間概念を導入せず、「ペアの組み替え」だけで全パターンが実現される。DX 層の apply_fn パターン（DR-042 で実証済み）と同じ構造が core に降りてくる形。
...
JSON schema + args → kuu core (WASM) → JSON result。Go/TypeScript/Python/Swift 等から利用可能。

**制約**: MoonBit は WASM-GC にコンパイルされるため、V8 ランタイム（Node.js, Deno, Bun）でのみ直接動作。非V8言語は Node.js サブプロセス経由の JSON ブリッジで接続（DR-033）。

### Opt 定義の AST 可搬性（DR-029, DR-030）

Opt 定義は純粋データ（定義レベルではクロージャなし）。JSON にシリアライズして他言語に転送可能:
...
positionals 配列の順番消費で serial を実現。`is_rest=true` なら同じハンドラを繰り返し使用。positional のハンドラは `--` prefix の引数を Reject する（has_prefix ヒューリスティクス）。
...
3. **4層レイヤーで複雑さを分離** — Core → Pattern → Convention → Sugar。各層は ExactNode の生成と登録という単一操作に統一
4. **Opt[T] は薄い参照型** — id + name + getter + parsed + is_set。値の格納はクロージャ内の Ref[T] に隠蔽
...
10. **core 無改造の原則** — 多言語 DX は dx → core の一方向依存。core は dx を知らない
...
| [DESIGN.md](DESIGN.md) | 原設計書（設計の進化の記録として保存） |
| `docs/decision-records/` | 個別の設計判断記録（DR-001〜DR-042） |
