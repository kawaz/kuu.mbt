...
alias コンビネータが `alias_sources` に登録。チェーン alias（alias の alias）時に `target_was_set` が root まで伝搬される。
...
register_option[T](name, shorts, aliases, variations, global, hidden, ...):
  1. wrap_node_with_set — was_set 追跡のラッパーを付与
  ...
  4. AliasSource を alias_sources に登録（alias チェーン用）
  5. Opt[T] を生成して返却
```
