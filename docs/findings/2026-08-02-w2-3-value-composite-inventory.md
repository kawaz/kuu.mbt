...
1. **W2-5 (産出者を通す) は本表を再訪する義務がある。** 特に C-3 / C-4 (`element_value*`) が
...
3. **A-3 と A-4 の綴りは wbtest では pin したが、どの fixture にも出ていない。** 産出者が入るまで
   conformance からは観測不能なので、W2-5 で最初の複合産出型を立てたときに両方を fixture 側へ
   露出させること (wbtest は実装内部の表明であって他実装が読む規範ではない)。
4. `ResultValue` と `Value` の統合是非は計画どおり W2-5。本段では両者の複合 arm が
   **独立に**書かれた状態になっている (A-3 が両方を跨ぐ唯一の関数)。
5. **`Value::Object` は重複キーを持てるが、素の JSON 綴り (A-3) は持てない。** 実測で
   `scalar_value_json(Object([("k",1),("k",2)]))` は `{"k":2}` に潰れる一方、`value_to_configval`
   (A-2) は両方の entry を保つ。したがって **encode/decode 往復が成立するのは重複キーの無い object に
   限る** (wbtest で境界ごと pin 済み)。複合を産む型を立てる W2-5 は、`parse_token` が重複キーを
   返せる設計にするのか、返せない不変条件を置くのかを決める必要がある。決めないまま産出者を入れると
   result と sources で field 数が食い違う。
