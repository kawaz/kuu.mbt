...
| 現名 (pkg) | 新名 (案) | 根拠 |
|---|---|---|
| `mkb` (engine) | `binding` または `mk_binding` | 名前から機能の手掛かりゼロ (コールドレビュー指摘)。`mk_eff` と対にするなら `mk_binding`/`mk_effect` で統一 |
| `mk_eff` (engine) | `mk_effect` | 略語解消、mkb と同時に |
...
| `InstallBuild` (engine) | `InstallOutput` (または `LoweredParts`) | 「er」有無だけの取り違えペア (コールドレビュー指摘は真の改善分)。InstallBuilder::output の戻り型なので出力を表す語へ |
| `pend_value` (engine) | `pending_value_candidate` (または `value_cand`) | pend の略語解消 + Pending Branch との関係を名前に |
| `cand_trigger` (engine) | `trigger_candidate` | 語順を英語法に (Cand 構築系で `*_candidate` に統一) |
