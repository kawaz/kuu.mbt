...
- `custom[T : Show]()`, `append[T]()` — 汎用型。string/int/float/boolean は custom のシュガー（DR-0025）
- `cmd()`, `sub()` — サブコマンド
...
ReduceCtx は struct ラッパー。MoonBit のクロージャにはラベル付き/オプション引数が使えないため、将来の拡張（フィールド追加）を壊さずに行うための設計判断（DR-0008）。
...
### FilterChain — Kleisli 合成による型安全な変換パイプライン（DR-0016）
...
- 数値変換: `clamp(min, max)`
- 選択: `one_of(allowed)`
- 配列: `each(inner_filter)`, `split(sep)`, `mergeable_list(base, separator)` — `+/-/...` 修飾子でベース相対変更（DR-0023）
- 正規表現: `regex_match(pattern)`, `regex_replace(pattern, replacement)`, `regex_split(pattern)`

**純粋性制約（DR-0037）**: フィルタ（pre/post/accum）は純粋関数であること。値の状態管理は `Ref[T]` で行い、フィルタは入力から出力への変換のみを担う。この制約により、clone 時にフィルタのクロージャ参照を安全に共有できる（直交プリミティブの前提条件）。

### 合成パターン — 直交プリミティブ（DR-0037）
...
core を各言語から利用する方式は未確定。言語ごとに最適な方式が異なり、ランタイムの成熟度にも依存する（DR-0033, DR-0046, DR-0047）:
...
### kuu-cli — Native CLI Transport（DR-0059、実装中）
...
- **embed パターン対応** (DR-0047): 各言語の DX レイヤーが kuu-cli バイナリを embed し、ユーザーからは通常のライブラリとして見える
- **独立コマンド** (DR-0057): シェルスクリプト・Makefile・CI/CD から直接利用可能
...
### Opt 定義の AST 可搬性（DR-0029, DR-0030 構想）
...
**Reject と Error の使い分け**は名前解決の前後で意味が変わる（DR-0015）:
...
消費ループの前に、特殊な引数形式を通常の ExactNode に変換する install ノードが parse_raw 冒頭で構築される（DR-0017）。これにより **parse_raw のメインループから特殊分岐が完全に排除**される:
...
**核心**: 各 ExactNode が「自分の消費可能性を自己判定できる」ため、ショートオプション結合の分解も install ノード内での再帰的 try_reduce 呼び出しとして表現される（DR-0015, DR-0039）。型情報（needs_value, consumed 数）がバックトラッキング判断に直接使われる。

### OC/P 2フェーズパース（DR-0034）
...
Opt は薄い参照型。initial / reducer / meta はコンビネータ内のクロージャにキャプチャされ、Opt 自体には含まれない。内部の `Accessor[T]` は ValCell から生成されたクロージャ束で、get/set/set_value/set_commit/reset の5操作を提供する（DR-0048 で is_set を Opt.used に分離）。ExactNode の try_reduce/commit と同じ ValCell を共有するため、型安全性は静的に保証される。
...
- **Opt.used**: この Opt 名が使われたかを示すクロージャ（DR-0048）。通常は `vc.committed` と同値、alias/clone では独立した `opt_used` フラグ
...
| [ROADMAP.md](ROADMAP.md) | 将来計画・未実装設計（エラー構造化、group、defaults マルチソース、多言語展開等） |
| [design/valcell-lifecycle.md](design/valcell-lifecycle.md) | ValCell/Accessor ライフサイクル詳細 |
| [design/kuu-essence.md](design/kuu-essence.md) | プロジェクトの本質・ポジショニング |
| [design/kuu-showcase.md](design/kuu-showcase.md) | ユースケース事例集（作成中） |
| `docs/decisions/` | 個別の設計判断記録（DR-0001 以降） |
