...
- 4層レイヤー構造: Core（ExactNode）→ Pattern（or）→ Convention（option）→ Sugar（flag 等）
...
- **parse は ExactNode のフラットリストを走査して ResultMap に結果を書き込む** — ExactNode の完全一致名マッチで型を知らずに操作
- **4層レイヤーで複雑さを分離** — Core（ExactNode + 消費ループ）→ Pattern（or）→ Convention（option）→ Sugar（flag 等）
...
- **try_reduce は TryResult を返す** — Accept（消費成功 + commit クロージャ）/ Reject（名前不一致）/ Error（名前一致だが値不正）。名前確定後の型変換失敗は即エラー
- **FilterChain[A, B] で String→T の前処理を型安全に合成** — map（純粋変換）/ validate（検証）/ parse（変換+失敗）の3コンストラクタ + `then` で Kleisli 合成。reducer シグネチャは変えずに複雑な変換・バリデーションを外出し可能（DR-016）
...
旧設計の `Opts` enum（Node/Array のツリー構造）は廃止。ExactNode のフラットリストと Positional 管理に置換。

パーサは `Array[ExactNode]` のフラットリストを保持する。各 ExactNode は完全一致の名前を持ち、消費ループは単純な線形走査で名前マッチを行う。

- **名前付きオプション** — ExactNode として直接リストに登録。`--verbose`, `--no-verbose` 等は個別の ExactNode
- **Positional** — パーサ内部の Positional 管理で順番消費を制御
- **serial** — 再帰的二項構造で Positional の活性化を連鎖（後述の合成パターン参照）
- **rest** — 可変長消費。Positional として登録

`opts([o1, o2, o3])` ヘルパーは各コンビネータが生成する ExactNode 群をフラットリストに展開する。

Design rationale (DR-012): 旧 Opts enum は JSON の Value のような再帰構造だったが、ExactNode が完全一致名を持つため
ツリー走査が不要になった。`--no-verbose` 等の反転パターンはコンビネータ層（Convention レイヤー）で独立した ExactNode に
展開されるため、消費ループは名前解決ロジックを持たなくて済む。プレフィックスマッチは MVP 後に追加レイヤーとして実装可能。

### 合成パターン

- **or** — 同一 Ref の複数 ExactNode を束ねる（Pattern レイヤー）。排他選択。子は自動で `required=false`
- **serial** — 再帰的二項構造: `serial(o1, o2, o3)` = `serial(o1, serial(o2, serial(o3)))`。「消費されたら次の positional を活性化する」という自然なリンクドリスト構造
- **group** — 繰り返し出現するオプション群。雛形を clone して各出現の値を保持
...
Design rationale (DR-012): 旧設計では serial は Opts::Array の特殊なバリアントだった。再帰的二項構造に変更することで、
各ノードの責務が明確になり、「消費されたら次を活性化」という振る舞いがリンクドリストとして自然に表現される。
...
- テスト: 535/535 passed
- **未実装**: `make_reducer` — ReduceCtx 未実装のため未着手
...
///| FilterChain + Accumulator → reducer の自動生成（未実装: ReduceCtx 未実装のため）
fn make_reducer[T, U](
...
### ParseResult ベースのアクセス（DR-013）

parse 後の結果は `ParseResult raise ParseError` として返される（`parse()` も `parse_raw` も raise 方式。ユーザー側で `try?` を使えば `Result` に変換可能）。2つのアクセス方法がある:
...
let result = p.parse(args)  // Result[ParseResult, ParseError] を返す

port.get()     // T? — スコープ内なら Some(T)、スコープ外なら None
verbose.get()  // T?
```

内部的には parserMap（`Map[Int, scope_info]`）を経由し、opt.id からスコープ内の結果を引く。
...
result.get(port)     // T? — 型付きアクセス
result.get(verbose)  // T?

// 階層ナビゲーション
result.child("serve")              // ParseResult? — サブコマンド結果
result.child("serve").get(port)    // T?
result.at(0)                       // ParseResult? — ポジショナル結果
```

#### ParseResult の型構造

MoonBit では enum にメソッドを定義できないため、enum を持つ struct として設計:
...
| メソッド | 戻り値 | 説明 |
|---------|--------|------|
| `Opt[T]::get()` | `T?` | cobra-style。parserMap 経由 |
...
コンテキストに応じた自然な使い分けが可能。`child`/`at` はプリミティブ（`as_map`/`as_list`）のシュガーで、
MoonBit のメソッドオーバーロード不可制約に対応して `get(Opt[T])` と名前ナビゲーションを分離した命名。

### Parser::get — 直接アクセス

`Parser::get(opt) -> T?` は parse 前なら `None`、parse 後はスコープ内なら `Some(T)`。

構築時に変数バインドし、parse 後に `parser.get(opt)` で型安全に取得する。
...
## ExactNode — 完全一致名マッチ + reducer（旧 ErasedNode の置換）

消費ループが走査するコアデータ構造。各 ExactNode は1つの完全一致名を持ち、名前がマッチしたら reducer を実行する。
...
pub(all) enum TryResult {
  ...
  // 消費成功。commit クロージャを呼ぶと ResultMap に書き込まれる
  Reject
  // 名前不一致。他の候補を試す
  Error(ParseError)
...
旧 ErasedNode との主な違い:
- **name フィールド**: 完全一致名を直接保持。旧設計の `meta.name` + children による名前解決が不要
- **needs_value**: 値を必要とするか否かを明示。消費ループが値引数の消費を判断できる
- **TryResult の3値**: 旧 TryReduceResult は Accept/Reject の2値だったが、Error を追加。exact match で名前確定後の型変換失敗は即エラーにすべき（Reject にすると別候補に流れて誤動作する）
- **commit クロージャ**: 旧設計の `commit_reduce` + `ReduceResult` を統合。Accept 内のクロージャが状態を閉じ込める
- **children 廃止**: ExactNode はフラットリストで管理されるため、ツリー走査用の children が不要
- **meta 廃止**: ExactNode レベルではメタ情報不要（名前が完全一致なのでメタ情報で名前解決する必要がない）

Design rationale (DR-012): 旧 ErasedNode は try_reduce/commit_reduce/children 等のクロージャ束でツリー構造を表現していたが、
ExactNode は完全一致名を持つフラットな構造。消費ループは単純な名前比較だけで済み、--no-xxx やエイリアスは
コンビネータ層で独立した ExactNode に展開される。trait object は MoonBit でダウンキャスト不可のため、
struct + クロージャで型消去を実現する方針は継承。
...
parse のコアは引数消費ループ。ExactNode のフラットリストに対して完全一致マッチを行う。2つのモードを持つ:
...
**初期化時（Parser::new / cmd）**:
1. **install_separator_node**: `--` を greedy ExactNode 化。残り args を positionals で serial 消費。`dashdash=true`（デフォルト）の場合に登録される

**parse_raw の冒頭**:
2. **install_eq_split_node**: `--name=value` 形式を分解する ExactNode を追加
3. **install_short_combine_node**: `-abc` 等のショートオプション結合を分解する ExactNode を追加

これにより parse_raw のメインループから特殊分岐が完全に排除される（DR-017）。

### アルゴリズム — ExactNode 完全一致マッチ

消費ループは ExactNode のフラットリストに対して完全一致の名前マッチを行う。旧設計のツリー走査・名前解決ロジックは不要。

```
0. カレントスコープ = パーサの ExactNode リスト（フラット）
   install_separator_node は初期化時に登録済み。install_eq_split_node / install_short_combine_node で特殊構文ノードを追加

1. カレント引数 args[i] を取得

2. 全 ExactNode に対して try_reduce(args, i) を呼ぶ:
   - TryResult::Accept(consumed, commit) → 候補残留
   - TryResult::Reject → 候補脱落
   - TryResult::Error(e) → 即座に ParseError

3. Accept した候補のうち consumed が最大のものを選ぶ:
   3a. 最大 consumed が複数 → ParseError（曖昧）
   3b. 最大 consumed が1つ → 勝者確定
   3c. Accept した候補が0 → Positional フォールバック（後述）

4. 勝者の commit() を呼んで状態に書き込み
   consumed 分だけ引数を shift
   消費ループ継続

5. Positional フォールバック（候補0の場合）:
   current_positional のインデックスで順番に消費
   rest でなければ次の positional に進む
   全 positional 消費済みなら ParseError
```

旧設計との主な違い:
- **名前解決不要**: ExactNode が完全一致名を持つため、消費ループ内での名前解決ロジックが不要
- **TryResult::Error**: 名前一致後の値不正を即エラーにできる（旧設計は Reject に変換していた）
- **commit クロージャ**: 旧設計の `commit_reduce(ReduceResult, ResultMap)` を簡略化。Accept 内のクロージャが状態を閉じ込める
- **子パーサ再帰**: スコープ切替は状態変数管理ではなく、子パーサの parse 再帰呼び出しで実現
...
- **ショートオプション結合の型情報分解**: `-cA1gcc` → c=3(count), A=1(int), g(flag)。各ショートの reducer が型を知っているため値の境界を自動判定
- **数値引数**: `kill -9` — positional の reducer が int パースを試みて Accept/Reject で判定
- **曖昧さ処理**: 複数の分解パターンが成立する場合は最長一致、それでも複数なら ParseError(ambiguous)

核心: **各 ExactNode が自分の消費可能性を自己判定できる**ため、ショートオプション結合の分解も消費ループの再帰適用として表現可能。詳細は DR-015 参照。
...
**結果アクセスの3つの口（DR-013）**:
- `parser.get(opt) -> T?`: parse 前は `None`、parse 後はスコープ内なら `Some(T)`
- `opt.get() -> T?`: cobra-style。parserMap 経由でスコープ判定
- `result.get(opt) -> T?`: ParseResult からアクセス。スコープ外なら `None`
- `result.child("serve") -> ParseResult?`: サブコマンドの結果にナビゲーション
「指定されたか」のより詳細な判定には `result.source(opt) -> ValueSource` を併用可能。
...
`--` は greedy な ExactNode として表現される（DR-017）。

**初期化時の自動登録**: `Parser::new(dashdash=true)` / `cmd(dashdash=true)` の初期化時に install_separator_node が `--` を ExactNode として自動登録（デフォルト ON）。`--` を検出したら残りの args を positionals で serial に消費し、P が終端したら Accept(consumed=N) で復帰する。force_positional 状態変数は不要。

**コンビネータ（DR-018、実装済み）**:
- `p.dashdash(separator?="--")` → `Opt[Array[String]]`: セパレータ以降の全 args を収集
- `p.append_dashdash(separator?="--")` → `Opt[Array[Array[String]]]`: セパレータで区切った複数グループを収集
...
- stop_before パラメータ、rest 先頭グループ化は未実装（DR-018 参照）
...
struct Parser {
  ...
  nodes : Array[ExactNode]   // このスコープの ExactNode フラットリスト
...
struct Opt[T] {
  ...
  setter : (Int, T) -> Unit  // instance_id を受け取り T を書き込む
...
- ルートパーサが `next_id: () -> Int` クロージャを生成
- 子パーサは `next_id` を共有し、ID 空間が統一される
- 各パーサは自スコープの `nodes`（ExactNode リスト）と `refs` を持つ
- コマンドの commit 時に子パーサの parse を残りの args で再帰呼び出し
- Ref[T] はクロージャ経由でどの階層からもアクセス可能

```moonbit
// 子パーサ生成
let child = Parser::child(parent.next_id)  // next_id を共有
// コマンドマッチ時: 残り args で子パーサを再帰呼び出し
child.parse(remaining_args)
```

**コマンドの非特別視（DR-012）**:

コマンドもオプションも「名前マッチ → 子 opts を活性化」する同じ構造。違いは kind による振る舞い:
- **Command**: 子パーサの parse を残り args で再帰呼び出し（スコープ置換）
- **Positional(serial)**: 次の positional を活性化

旧設計では parse 関数内でスコープ状態変数を管理していたが、子パーサ方式により各パーサがスコープ単位の refs を自然に持つ。

**全 ID は同一 seq（next_id クロージャ）から採番**（デバッグ時の混乱防止）:
```
...
cobra-style の `opt.get() -> T?` を実現するため、Parser が `Map[Int, scope_info]` を保持。
opt.id からそのスコープの parser/parseresult を引き、スコープ内なら `Some(value)`、スコープ外なら `None` を返す。
子パーサは parserMap を共有するため、どの階層で定義された opt からも正しいスコープの値にアクセスできる。
...
let result = p.parse(args)           // Result[ParseResult, ParseError] を返す
p.get(port)                          // T? — parse 後ならスコープ内で Some
port.get()                           // T? — cobra-style
result.get(port)                     // T? — ParseResult からアクセス
...
- Parser ローカル seq（next_id クロージャ）でテスト分離・並行処理対応
...
| `--` (double dash) | greedy ExactNode。残り args を positionals で serial 消費（DR-017） |
| OC モード | 初期モード。Option/Command 優先、Positional はフォールバック |
| P モード | OC 全滅後。Option 割り込み可（スコープ認識局所分解含む）、Command 不可 |
| 結果保持 | `ParseResult`（階層ナビゲーション可能。child/at/get）+ `ResultMap`（内部実装） |
| cobra-style アクセス | `opt.get() -> T?`（parserMap 経由でスコープ判定） |
...
| FilterChain | `FilterChain[A, B]` — `then` で Kleisli 合成。`map`/`validate`/`parse` の3コンストラクタ【実装完了】 |
| Accumulator | `(T, U) -> T` — 片 U を累積型 T にマージ。`make_reducer` で reducer に接続（`make_reducer` は未実装） |
...
以下は実装計画の Step とは独立に実装された機能:

- **FilterChain[A, B]** — 型安全な変換パイプライン（`src/core/filter.mbt`）【実装完了】
  - `FilterChain::then`（Kleisli 合成）、3コンストラクタ、9組み込みパーツ、`Accumulator[T, U]` 型
  - `make_reducer` は ReduceCtx 未実装のため未着手
- **parse() Result化** — `Parser::parse()` が `Result[ParseResult, ParseError]` を返すように変更【実装完了】
  - `parse_raw` は内部関数のため `raise` のまま維持
  - 全テスト更新済み、535/535 passed
...
- `--` separator は Parser::new/cmd の初期化時に install_separator_node で自動構築（dashdash=true の場合）（DR-017）
...
> **注意**: 以下の拡張フェーズ（Step 4〜12）は実装検証前の設計案である。MVP（Step 0〜3）の実装結果を踏まえて見直す。
...
opt.get()               // -> T?       cobra-style（parserMap 経由）
