...
| Convention | expand_and_register — --name + variations 展開 | parser.mbt |
| Sugar | flag(), string_opt(), int_opt(), count(), positional(), rest(), cmd() | options.mbt, commands.mbt, positionals.mbt, dashdash.mbt |
...
```
引数入力 → [validate_no_duplicate_names] → [install ノード構築] → [消費ループ] → [post_hooks]
```
...
| サブコマンド | `p.cmd(name=..., setup=fn)` / `p.sub(name, fn(child) { ... })` → `Opt[Bool]` + 子 Parser | 実装済み |
...
| sub() | `p.sub(name, fn(child) { ... })` — cmd のシュガー | 実装済み |
...
| require_cmd | `parser.require_cmd()` — サブコマンド必須制約 | 実装済み |
...
- **コンビネータ**: flag, string_opt, int_opt, count, append_string, append_int, positional, rest, cmd, sub, dashdash, append_dashdash
...
- **sub()**: cmd のシュガー。`p.sub(name, fn(child) { ... })` で子パーサ構築
