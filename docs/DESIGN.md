...
- **alias** — 汎用別名コンビネータ。clone + link で Ref 共有する別エントリを作る。opt / cmd / positional 問わず使える（DR-037、未実装）
- **proxy** — 介入可能ラッパー。alias + commit wrap で deprecated 警告・使用検知等の副作用を差し込む（DR-037、未実装）
...
| Core | ExactNode（完全一致名マッチ + try_reduce）+ 消費ループ + プリミティブ（exact, clone, link） | parse.mbt, nodes.mbt |
| Pattern | make_or_node, alias, proxy, variation, require_prefix — 合成パターン | nodes.mbt |
...
複合パターンを単一の ExactNode に見せる。DR-037 で alias / proxy プリミティブを導入し、
name / aliases の偽の区別を解消。alias = clone + link、proxy = alias + 介入フック。
...
- **alias / proxy プリミティブ (DR-037)** — alias = clone + link（汎用別名）、proxy = alias + 介入フック（deprecated 警告等）。aliases パラメータは alias のシュガーに。name は表示ラベルに限定。単一ダッシュロングオプション（`-Xcc` 等）も自然に解決
...
| proxy | `p.proxy(name, target, deprecated=...)` — 介入可能ラッパー（DR-037） | 未実装 |
