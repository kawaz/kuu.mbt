...
parse_raw の冒頭で3つの install メソッドが呼ばれ、特殊構文を通常の ExactNode に変換する:

1. **install_separator_node**: `--` を greedy ExactNode 化。残り args を positionals で serial 消費
...
0. カレントスコープ = パーサの ExactNode リスト（フラット）
   install_separator_node / install_eq_split_node / install_short_combine_node で特殊構文ノードを追加
...
**現在の実装**: install_separator_node が parse_raw の冒頭で `--` を ExactNode として自動登録。`--` を検出したら残りの args を positionals で serial に消費し、P が終端したら Accept(consumed=N) で復帰する。force_positional 状態変数は不要。

**将来の設計（DR-018）**: dashdash / append_dashdash コンビネータとして sugar 化。
- `p.dashdash()` — 単発。P を1回消費
- `p.append_dashdash()` — 複数グループ。`Array[P の serial の結果型]` を返す
- `stop_before` パラメータで greedy 系の停止条件を制御
- セパレータは `--` に限らず任意の文字列を指定可能
...
- `--` separator は install_separator_node で ExactNode として自動構築（DR-017）
