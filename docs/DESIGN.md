...
parse 後の結果は `Result[ParseResult, ParseError]` として返される（`parse()` は Result 型、内部の `parse_raw` は raise 方式）。2つのアクセス方法がある:
...
2. 全 ExactNode に対して名前の完全一致を判定:
   - force_positional が true の場合、Option 系の ExactNode をスキップ
   - node.name == args[i] → try_reduce(args, i) を呼ぶ
   - TryResult::Accept(consumed, commit) → 候補残留
   - TryResult::Reject → 候補脱落
   - TryResult::Error(e) → 即座に ParseError（名前一致だが値不正）

3. greedy フィルタ
   残留候補に greedy=true があれば greedy=false を除去

4. Accept した候補が0:
   4a. args 残りなし → パース完了。消費ループ脱出
   4b. Positional なし → ParseError（予期しない引数）
   4c. Positional あり → P モードに遷移して消費ループ継続

5. Accept した候補のうち consumed が最大のものを選ぶ:
   5a. 最大 consumed が複数 → ParseError（曖昧）
   5b. 最大 consumed が1つ → 勝者確定

6. 勝者の commit() を呼んで ResultMap に書き込み
   consumed 分だけ引数を shift
   消費ループ継続
   - Command の場合: 子パーサの parse を残り args で再帰呼び出し（スコープ切替）
   - name="" + greedy + global の Flag の場合 → force_positional = true
```
