...
- **adjust** — フィルタ挿入。alias なしでも単独使用可。LongVariation（Toggle/True/False/Reset/Unset）は adjust の引数プリセット定数。deprecated も adjust の before_accum で記録するパターンとして実現【未実装】
...
- **alias / adjust プリミティブ (DR-037)** — 3 つの直交プリミティブ: clone（アイデンティティ）、link（値共有）、adjust（振る舞い変更）。alias = clone + link。variation = alias + adjust。deprecated = adjust の記録パターン。aliases パラメータは alias のシュガーに。単一ダッシュロングオプション（`-Xcc` 等）も自然に解決
...
| deprecated | `p.deprecated(name, target, msg)` — adjust の記録パターン。パース後に警告確認（DR-037） | 未実装 |
...
| 最低1つ必須 | `parser.at_least_one(opts)` | 未実装 |
...
#### Step 4: 制約チェック（post_hooks ベース）【一部実装済み】

- `parser.exclusive(opts)` — 排他オプション【実装済み】
- `parser.at_least_one(opts)` — 最低1つ必須【未実装】
...
基本的な排他制約は `parser.exclusive(opts)` で実装済み。`parser.at_least_one(opts)` は未実装。`parser.required(opt)` で単一オプションの必須化は実装済み。
以下は当初の設計案と追加検討事項。
