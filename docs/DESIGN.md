...
- **try_reduce は投機的実行のため例外を投げない** — 消費ループは全候補に対して投機的に try_reduce を呼び、最長一致を選ぶ。非採用候補のエラーで全体が落ちないよう、try_reduce は raise しない。reducer 内部の ParseError は try? で catch し Reject を返す
...
```moonbit
///| リーフノード — 定義情報 + getter クロージャで型消去を解決
struct Opt[T] {
  id : Int                    // 一意 ID（Parser.seq から採番）
...
impl[T] Hash for Opt[T] with hash(self) { self.id }
...
Design rationale: Opt[T] は完全 immutable（getter クロージャの参照先が mutable なだけ）。
値の管理は Parser struct に一元化され、getter クロージャ内に隠蔽される（後述の Parser struct + getter 方式を参照）。
...
- **`--` (double dash)** — `flag(name="", greedy=true, global=true)` として表現。消費ループで Accept すると force_positional 状態に遷移し、以降の引数を全て positional として扱う。serial と組み合わせて使用
...
pub(all) struct ErasedNode {
  meta : OptMeta
  try_reduce : (args : Array[String], pos : Int) -> TryReduceResult
...
pub(all) enum TryReduceResult {
  Accept(consumed~ : Int, result~ : ReduceResult, quality~ : MatchQuality)
  Reject
}
...
```
try_reduce(args, pos):
  1. match_name(args[pos], self.meta) → Matched(action, quality) | NotMatched
  2. NotMatched なら Reject を返す
  3. action に基づいて値消費を決定（= 付き値 or 次引数消費）
  4. reducer(ReduceCtx { current, action }) を try? で呼ぶ
     - reducer が ParseError を raise した場合 → catch して Reject を返す
     - reducer が None を返した場合 → Reject を返す
     - reducer が Some(T) を返した場合 → 次へ
  5. Accept(consumed, result, quality) を返す
```

注: try_reduce は投機的実行のため raise しない。reducer の `(ReduceCtx[T]) -> T? raise ParseError` シグネチャ自体は変えないが、try_reduce 内部で `try?` により catch し、エラー時は Reject として扱う。これにより非採用候補のエラーが消費ループ全体を落とすことを防ぐ。commit_reduce は確定後の操作なので raise 可能のまま。
...
///| 名前マッチング結果
enum MatchResult {
  Matched(action : ReduceAction, quality : MatchQuality)  // マッチ、action + 一致品質
  NotMatched
}

///| マッチの品質 — 完全一致 vs プレフィックスマッチ
enum MatchQuality {
  Exact    // 完全一致（--output が --output にマッチ）
  Prefix   // プレフィックスマッチ（--out が --output にマッチ）
}
...
- ショートオプション: `-n`
- エイリアス完全一致
- プレフィックスマッチ（曖昧性の判定は消費ループ側で行う）
- 反転パターン: `--no-name`, `--enable-name`, `--disable-name`
- `=` 付き値分解: `--port=8080` → `Matched(Value(Some("8080")))`
- サブコマンド名マッチ

**プレフィックスマッチの扱い**: 各ノードの match_name はプレフィックスマッチで `Matched(action, quality=Prefix)` を返し、完全一致で `Matched(action, quality=Exact)` を返す。消費ループが全ノードの結果を比較し、Exact マッチがあれば Prefix のみのノードを除外する。Prefix マッチのみが複数残った場合は AmbiguousOption エラーを出す。
...
**`ctx.get(other_opt)` のセマンティクス（stale read）**:

- 投機実行中（try_reduce 内）の `ctx.get(other_opt)` は、**前の iteration で commit_reduce により確定した値のみ**を参照できる
- 現在の try_reduce ラウンドで他の候補が出した値は見えない（各候補の try_reduce は副作用なしで並行的に実行されるため）
- commit_reduce 完了後に初めて最新値が Parser.refs に反映される
- したがって reducer 内の `ctx.get()` は「1ステップ前の確定済み状態」を見ることになる（stale read）
...
- **P モードでも短縮オプション展開を適用**: `-abc` が OC ノードにそのままマッチしない場合、スコープ Reducer が `-a`, `-b`, `-c` への展開を試みる（OC モードの短縮オプション展開と同じメカニズム）
...
PoC 検証の結果、**Parser struct + getter 方式**を採用（後述の Parser struct セクション参照）。ResultMap は ID + clone レジストリのみを保持し、値は Parser.refs に一元管理される。
...
**値の管理は Parser に一元化**。Opt[T] の getter クロージャが Parser.refs 内の Ref[T] をキャプチャし、instance_id を受け取って型付きの値を返す。

**型安全な値の取得**:

```moonbit
///| Parser 経由で Opt[T] をキーとして型安全に値を取得
fn get[T](self : Parser, opt : Opt[T]) -> T {
  opt.getter(self.root_result_id)  // getter が Ref[T] から直接 T を返す。ダウンキャスト不要
}

///| サブコマンド取得
fn command(self : Parser) -> ErasedNode?  // None = トップレベル or サブコマンド以外
```
...
fn clone(self : ResultMap, parser : Parser) -> ResultMap {
  let new_id = parser.next_id()
...
(2) **型ありビュー** — `Opt[T].getter(instance_id)` で直接 T として取り出す
最初から T を知っている `Opt[T]` と、T を閉じ込めたクロージャが同じ `Ref[T]` を共有するため、型安全性は静的に保証される。ダウンキャスト不要。
ref 管理が Parser に一元化されることで、メモリリーク問題が構造的に解消される。
...
`parser.get(port)` は内部的に `port.getter(parser.root_result_id)` で直接 T を返す。
...
**force_positional の仕組み**:

消費ループ側での `--` 検出と状態遷移:

1. `--` は `flag(name="", greedy=true, global=true)` として表現（上記）
2. 消費ループで Accept したノードが `kind=Option, name="", greedy=true, global=true` の Flag であれば、`force_positional = true` に遷移
3. `force_positional` 状態では OC モードの名前マッチングをスキップし、全引数を P モード（positional）として処理
4. `force_positional` の効果範囲: 現在のスコープおよびスコープ遷移後も持続（一度 `--` が出現したら以降全て positional）

Design rationale: greedy + serial の組み合わせで `--` を通常の Opt として表現することで、
特殊処理が不要になり、`--exec` 的な「以降全て引数」パターンにも同じ仕組みで対応できる。
force_positional は消費ループの状態フラグとして実装し、`--` ノードの Accept を検出した時点で遷移する。
...
- `TryReduceResult` — Accept(consumed, result, quality) / Reject
...
- `MatchResult` — Matched(action, quality) / NotMatched
- `MatchQuality` — Exact / Prefix（完全一致 vs プレフィックスマッチの品質）
