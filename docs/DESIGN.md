...
- 4層レイヤー構造: Core（ExactNode）→ Pattern（make_or_node）→ Convention（register_option）→ Sugar（flag 等）
...
- **Opt[T] は薄い参照型** — id + getter + parsed のみ保持。値の格納はクロージャ内の Ref[T] に隠蔽
- **parse は ExactNode のフラットリストを走査** — 完全一致名マッチ + 最長一致で候補選択。型を知らずに操作
- **4層レイヤーで複雑さを分離** — Core（ExactNode + 消費ループ）→ Pattern（make_or_node）→ Convention（register_option）→ Sugar（flag 等）
...
Opt 自体には含まれない。これにより Opt のフィールドは最小限に保たれ、ユーザーが保持しても負担にならない。
...
pub fn[T] Initial::resolve(self : Initial[T]) -> T {
  match self { Val(v) => v; Thunk(f) => f() }
}
```

Design rationale: `Initial[T]` は即値と遅延評価を統一的に扱う。implicit_value のデフォルト値や
...
Design rationale (DR-012): ExactNode が完全一致名を持つためツリー走査が不要。`--no-verbose` 等の反転パターンは
コンビネータ層（Convention レイヤー）で独立した ExactNode に展開されるため、消費ループは名前解決ロジックを持たなくて済む。
...
- **serial** — 再帰的二項構造で Positional の活性化を連鎖。現状は positionals 配列の順番消費で代替
- **group** — 繰り返し出現するオプション群。雛形を clone して各出現の値を保持（未実装）
- **never** — 常に ParseError を投げるセンチネル（未実装）

#### 4層レイヤー構造

```
flag(name="verbose")                   # Sugar: Bool特化
  → register_option(node, no_node)     # Convention: --name/--no-name パターン生成
    → make_or_node([                   # Pattern: 複数 ExactNode を最長一致で統合
        ...
        make_default_fallback_node(),  # Core: consumed=0（finalize パスで commit）
      ])
```

| レイヤー | 責務 | 実装ファイル |
|---------|------|------------|
| Core | ExactNode（完全一致名マッチ + try_reduce）+ 消費ループ | parse.mbt |
| Pattern | make_or_node — 複数 ExactNode を最長一致で統合 | combinators.mbt |
| Convention | register_option — --name/--no-name 生成、choices/implicit_value 展開 | combinators.mbt |
| Sugar | flag(), string_opt(), int_opt(), count(), positional(), rest(), cmd() | combinators.mbt |
...
| Single(String) | `string_opt()` | make_reducer(identity, replace, initial) |
| Single(Int) | `int_opt()` | make_reducer(parse_int, replace, initial) |
...
| `make_default_fallback_node` | 0 | finalize パスで commit（未確定のデフォルト） |
| `make_or_node` | 最大 | コンポジット（子ノードの最長一致） |
| `make_reduced_value_node` | 2 | reducer 経由の値ノード |
| `make_reduced_negate_node` | 1 | reducer 経由の negate ノード |
...
**初期化時（Parser::new / cmd）**:
1. **inject_help_node**: `--help` / `-h` を ExactNode として登録。HelpRequested エラーを返す
2. **install_separator_node**: `--` を ExactNode 化。残り args を positionals で serial 消費。`dashdash=true`（デフォルト）の場合に登録される

**parse_raw の冒頭**:
3. **install_eq_split_node**: `--name=value` 形式を分解する ExactNode を追加。consumed>=2 gate で implicit_value の誤マッチを防止
4. **install_short_combine_node**: `-abc` 等のショートオプション結合を分解する ExactNode を追加。value trial (needs_value=true, consumed>=2) → flag trial のフォールバック
...
4. consumed=0 finalize パス（メインループ終了後）:
   全 ExactNode を1回試行し、consumed=0 の Accept を返すノードの commit を呼ぶ
   → make_default_fallback_node が未確定のデフォルト値を commit する

5. parsed フラグを true に設定

6. post_hooks を順に実行（値変換・遅延バリデーション）
```
...
現在の実装では各コンビネータの `default` パラメータで初期値を指定する方式のみ。
...
pub(all) struct OptMeta {
  ...
  default_display : String // デフォルト値の表示文字列。空なら表示しない
  global : Bool            // global=true ならヘルプで "Global Options" セクションに表示
  short : Char?            // ショートオプション（Some('p') 等）
...
7. **choices のヘルプ表示** — choices は OptMeta に含まれないため、ヘルプに候補一覧を表示する仕組みが未実装
...
**メリット**:
- Opt[T] は id + getter + parsed の3フィールドのみ（最小限）
...
- **mutual exclusion** — post_hooks ベースの制約チェック
- **dependent options** — post_hooks ベースの制約チェック
- **リザルト取得サポート** — シンプル JSON 出力等
- **ヘルプ生成の拡張** — 3段階カスタマイズ（調査レポート: help-format-survey.md）
...
```
引数入力 → [install ノード構築] → [消費ループ] → [consumed=0 finalize] → [post_hooks]
```
...
| **finalize** | consumed=0 の Accept を commit（デフォルト値確定） | 実装済み |
...
| サブコマンド | `p.cmd(name=..., setup=fn)` → `Opt[Bool]` + 子 Parser | 実装済み |
...
| choices | `p.string_opt(choices=[...])` | 実装済み |
...
| consumed=0 finalize | メインループ後にデフォルト値 commit | 実装済み |
...
| Initial[T] | `Val(T)` / `Thunk(() -> T)` — 遅延評価可能な初期値 | 実装済み |
| ヘルプ生成 | `generate_help()` — Commands / Options / Global Options | 実装済み |
| hidden | `hidden=true` — ヘルプ非表示 | 実装済み |
| 結果保持 | `ParseResult`（階層ナビゲーション可能。child/at/get） | 実装済み |
| 繰り返しグループ | `group(name, tmpl)` — 雛形 clone | 未実装 |
| 排他オプション | `parser.exclusive([...])` | 未実装 |
...
src/
  core/           # 全機能を統合（型定義 + コンビネータ + パースロジック + ヘルプ + フィルタ）
    types.mbt      # Opt[T], Parser, ExactNode, TryResult, OptMeta, ParseResult, Initial[T] 等
    combinators.mbt # flag, string_opt, int_opt, count, positional, rest, cmd, dashdash 等
    parse.mbt      # parse_raw, install_eq_split_node, install_short_combine_node, install_separator_node
    help.mbt       # generate_help, inject_help_node, format_opt_left
...
- **型定義**: Opt[T], Parser, ExactNode, TryResult, OptMeta, ParseResult, ParseResultKind, OptKind, Initial[T], ReduceCtx[T], ReduceAction, FilterChain[A, B], Accumulator[T, U], ParseError, ParseErrorInfo
- **コンビネータ**: flag, string_opt, int_opt, count, append_string, append_int, positional, rest, cmd, dashdash, append_dashdash
- **パースロジック**: parse_raw（メインループ + consumed=0 finalize + post_hooks）、install_eq_split_node, install_short_combine_node, install_separator_node
...
- **short option**: flag/string_opt/int_opt/count/append_string/append_int に short パラメータ追加
...
- **post hooks**: string_opt に post パラメータ（FilterChain）追加
...
現在は `src/core/` に全機能を統合。ファイル分割で責務を分離:

```
src/core/
  types.mbt            # 型定義（Opt, Parser, ExactNode, TryResult, OptMeta 等）
  combinators.mbt      # コンビネータ（flag, string_opt, int_opt, count, rest, cmd 等）
  parse.mbt            # パースロジック（parse_raw, install_* ノード）
  help.mbt             # ヘルプ生成（generate_help, inject_help_node）
