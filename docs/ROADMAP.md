...
Levenshtein 距離ベース。grapheme cluster 対応済み（DR-0049）。

### 値コンビネータ追加

- `boolean` — `true/false/yes/no/1/0/t/f/on/off` をパース（`Filter::parse_bool`）
- `float` / `append_float` — `Double` 型の値コンビネータ（DR-0054）

### 制約コンビネータ追加

- `deprecated` — 非推奨別名。alias + post_hook 方式で実装（DR-0040）
- `at_least_one` — 排他の対称。指定グループから最低1つ必須（DR-0040）
- `requires` — 依存制約。OptRef 間の依存関係を宣言

### clone / link / adjust / make_alias（DR-0037, DR-0044, DR-0045）
...
### WASM bridge 機能ギャップ解消（DR-0056）

DR-0051 で指摘された4件の未対応機能を実装:
...
> **注**: ロードマップ旧版では4段階（Deprecated を含む）を構想していたが、`deprecated` は独立コンビネータとして実装済み（DR-0040）のため、Visibility enum には含めない設計とした。
...
### sub_parser_combinator 抽象化（DR-0039）
...
**ブロッカー**: group の消費モデル設計（clone / link / adjust プリミティブは実装済み — DR-0045）
...
| **protobuf/gRPC** | IDL → 多言語コード生成 | データシリアライズ。CLI パーサではない。「スキーマから多言語生成」は kuu の Opt AST 可搬性（DR-0029）と同パターン |
...
3. Opt 定義が純粋データで JSON にシリアライズ可能（DR-0029, DR-0030）
...
| **wasmtime** (v27+) | **対応済み** | **対応済み** | **未統合** | **DR-0033 から状況変化**。Rust/Python で直接ロード可能に |
| **wazero** (Go) | **未対応** | 未対応 | — | Go は引き続き Node.js サブプロセスが必要 |
| **WasmKit** (Swift) | 未対応 | 部分対応 | — | |
| **Wasmer** | V8 バックエンド経由のみ | 部分 | — | |

**重要な変化（DR-0033 以降）**: wasmtime v27 が WASM-GC をサポート。これにより **Rust と Python は Node.js サブプロセスなしで kuu の WASM-GC バイナリを直接ロード可能**になった。ただし Canonical ABI がリニアメモリ前提のため、Component Model + WASM-GC の統合はまだ。
...
**ステータス**: 初回調査完了。wasmtime WASM-GC サポートは DR-0033 の更新が必要。定期的にランタイムサポート状況を追跡する
...
### KuuCore — 言語別 SDK 共通基盤（DR-0036 構想）
...
**ブロッカー**: FFI 調査（WASM bridge は DR-0035 + DR-0056 で主要機能対応済み）

### WASM bridge 拡張

DR-0035 で主要機能の PoC を実装。DR-0056 で機能ギャップ4件（env, at_least_one, requires, deprecated）を解消済み。残りの拡張:
...
| FFI 連携ベストプラクティス | 各言語から kuu core を呼ぶ最適な方法の確立 | 本ドキュメントに記載 | **初回調査完了** — wasmtime WASM-GC 対応が判明。DR-0033 更新が必要 |
