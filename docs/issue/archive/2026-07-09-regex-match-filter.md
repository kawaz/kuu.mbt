...
pending_entered:
discarded_entered:
resolved_entered: 2026-07-10T20:18:27+09:00
discard_reason:
pending_reason:
close_reason: ["dr/DR-085", "implemented"]
blocked_by:
origin: kuu (spec リポ)
---
...
## 2026-07-10 close: 実装完了

調達方針 = MoonBit core string の第一級 Regex の薄い wrapper (方言 = core Regex 準拠、
DR-085 §2)。`filters_registry` に `regex_match` 追加 (kuu.mbt commit `d7909136`、
filters.mbt:128-165)。spec fixture `fixtures/piece-filters/regex-match.json` 8 case
全 green (輪郭 4 つ + colon オブジェクト形式)。conformance decoded=170/mismatches=0。
compile 失敗は不一致同様 reject (kawaz 裁定 2026-07-10、DR-085 §1)。colon 対応はオブジェ
クト形式 (DR-085 §3) で解決。内部表現 (U+E000 タグ encode) の `FilterSpelling` 型再設計は
別 issue で追跡。
