...
- [x] spec 側で multiple×ref の意味論が fixture 化される (spec issue 参照)
- [x] accumulator が ref 要素にも配線され row が累積される (または明示 reject)
- [x] conformance green

## 解決

DR-084 (spec docs/decisions/DR-084-multiple-ref-row-accumulation.md) で意味論確定
(発火値=row append / repeat×multiple 発火境界保存 / merge×ref definition-error)。
kuu.mbt commit 534d10f1 で accumulator を配線し実装。fixture 3 本 green、
