# Issue Index

| date | category | status | slug | 概要 |
|---|---|---|---|---|
| 2026-07-07 | design | open | [accum-export-key-rename-asymmetry](./2026-07-07-accum-export-key-rename-asymmetry.md) | accum セルの export_key rename が未対応 (resolve 層の非対称) |
| 2026-07-08 | bug | open | [min0-lazy-repeat-gap](./2026-07-08-min0-lazy-repeat-gap.md) | min:0+lazy:true の unbounded repeat で laziness が沈黙で greedy に落ちる (Many lowering に lazy の座席なし) |
| 2026-07-08 | bug | open | [greedy-once-and-idxrepeat-premises](./2026-07-08-greedy-once-and-idxrepeat-premises.md) | greedy 1回制約・IdxRepeat bounded-head 前提・tried_triggers 失敗位置の追跡 (deep review low/medium 3件) |
| 2026-07-08 | design | open | [export-key-collision-identity-exposure-gap](./2026-07-08-export-key-collision-identity-exposure-gap.md) | find_export_collisions が identity 露出 (ek 未登録) 由来の衝突を見逃す |
| 2026-07-08 | design | open | [pending-starvation-missing-operand-path](./2026-07-08-pending-starvation-missing-operand-path.md) | Pending 飢餓経由の missing_operand が DR-066 §4 path を持てない |
| 2026-07-08 | design | open | [dec-alias-long-override-flag-sugar-gap](./2026-07-08-dec-alias-long-override-flag-sugar-gap.md) | alias long_override の long:true が DR-076 §2 の flag 糖衣差し替えを受けない |
